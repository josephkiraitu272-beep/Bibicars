...
const METHOD_VISUAL = {
  ...
  apple_pay: {
    label: "Apple Pay",
    hint: "One-tap on Safari / iOS",
    accent: "#000000",
    logo: "",
  },
  google_pay: {
    label: "Google Pay",
    hint: "One-tap on Chrome / Android",
    accent: "#4285F4",
    logo: "G·Pay",
  },
  link: {
    label: "Link",
    hint: "Stripe one-click checkout",
    accent: "#00D924",
    logo: "Link",
  },
  klarna: {
    label: "Klarna",
    hint: "Pay in 4 instalments",
    accent: "#FFB3C7",
    logo: "Klarna",
  },
  afterpay_clearpay: {
  ...
  crypto: {
    ...
    hint: "USDC stablecoin (Stripe Crypto)",
    accent: "#F7931A",
    logo: "₿",
  },
  us_bank_account: {
...
export default function PaymentMethodPicker({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`${API_URL}/api/stripe/public-config`)
      .then((res) => {
        if (!cancelled) {
          setConfig(res.data);
          if (res.data?.displayMethods?.[0])
            setSelected(res.data.displayMethods[0].key);
        }
      })
      .catch(() => {
        if (!cancelled) setConfig({ enabled: false, displayMethods: [] });
      })
      .finally(() => {
              ...
              {methods.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${active ? "border-[#635BFF] bg-[#635BFF]" : "border-gray-300 bg-white"}`}
                    >
                      {active && (
