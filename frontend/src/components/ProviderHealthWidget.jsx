...
export default function ProviderHealthWidget({ className = "" }) {
  ...
  if (loading) {
    return (
      <div
        className={`rounded-2xl bg-zinc-100 h-36 animate-pulse ${className}`}
        data-testid="provider-health-widget-loading"
      />
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <div
            ...
            data-testid="provider-health-score"
          >
            {score}
            <span className="text-xl opacity-70">/100</span>
          </div>
          <div
            className="mt-1 text-xs uppercase tracking-wider font-semibold opacity-90"
            data-testid="provider-health-tier"
          >
            {stats.tier}
          </div>
        </div>
        <div className="text-right text-xs opacity-80">
          <div>
            {t("r9_orders_label")}:{" "}
            <b className="text-white">{stats.metrics?.totalOrders ?? 0}</b>
          </div>
          <div>
            {t("r9_completed_label")}:{" "}
            <b className="text-white">{stats.metrics?.completedOrders ?? 0}</b>
          </div>
          {stats.metrics?.responseTimeAvg !== null &&
            stats.metrics?.responseTimeAvg !== undefined && (
              <div>
                {t("r9_avg_start_label")}:{" "}
                <b className="text-white">
                  {stats.metrics.responseTimeAvg} {t("r9_min_short")}
                </b>
              </div>
            )}
        </div>
      ...
      <div
        ...
        data-testid="provider-health-message"
      >
