  ...
  switch (n.i18n_key) {
    case "notif_new_lead":
      ...
      body =
        ...
        (p.source && p.source !== "manual"
          ? ` · ${tt("notifSource", "source")}: ${p.source}`
          : "") +
    ...
    case "notif_bulk_in":
      title = interp(
        tt(
          "notifBulkIn",
          "You have been assigned {count} {entity}(s) by {actor}",
        ),
      );
      body = interp(
        tt("notifBulkInBody", "{count} transferred · {tasksMoved} open tasks"),
      );
      break;
    case "notif_bulk_out":
      title = interp(tt("notifBulkOut", "Transferred to manager {target}"));
      body = interp(
        tt("notifBulkOutBody", "{count} {entity}(s) · {tasksMoved} tasks"),
      );
      break;
    case "notif_sla_warning_mgr":
      title = interp(
        tt("notifSlaWarningMgr", "Lead SLA warning ({minutes} min)"),
      );
      body = interp(
        tt(
          "notifSlaWarningMgrBody",
          "Lead «{lead}» still has no first response after {minutes} min.",
        ),
      );
      break;
    case "notif_sla_warning_tl":
      title = interp(
        tt(
          "notifSlaWarningTl",
          "Team SLA warning: {minutes} min without response",
        ),
      );
      body = interp(
        tt(
          "notifSlaWarningTlBody",
          "{manager} has not responded to lead «{lead}» yet.",
        ),
      );
      break;
    case "notif_sla_escalated_mgr":
      title = interp(
        tt(
          "notifSlaEscalatedMgr",
          "Lead escalated — SLA breach ({minutes} min)",
        ),
      );
      body = interp(
        tt(
          "notifSlaEscalatedMgrBody",
          "Lead «{lead}» has no response. Escalated to your team lead.",
        ),
      );
      break;
    case "notif_sla_escalated_tl":
      title = interp(
        tt("notifSlaEscalatedTl", "Lead escalated to you (SLA breach)"),
      );
      body = interp(
        tt(
          "notifSlaEscalatedTlBody",
          "Lead «{lead}» owned by {manager} hit the {minutes}-min escalation threshold.",
        ),
      );
      break;
    default:
...
const NotificationBell = () => {
      ...
      try {
        if (
          ...
          const n = new Notification(
            loc.title || notification.title || "BIBI CRM",
            {
  ...
  const formatTime = (dateString) => {
    ...
    if (diff < 3600)
      return `${Math.floor(diff / 60)} ${t("minutesAgo") || "min ago"}`;
    if (diff < 86400)
      return `${Math.floor(diff / 3600)} ${t("hoursAgo") || "h ago"}`;
  ...
  return (
    <div className="relative" ref={dropdownRef} data-testid="notification-bell">
      ...
      >
        <Bell
          size={22}
          weight={unreadCount + wave18Unread > 0 ? "fill" : "regular"}
        ...
        {unreadCount + wave18Unread > 0 && (
          ...
          >
            {unreadCount + wave18Unread > 99
              ? "99+"
              : unreadCount + wave18Unread}
          </motion.span>
        )}
            ...
            {wave18Unread > 0 && (
              ...
              >
                <span className="text-sm font-semibold text-indigo-900">
                  {wave18Unread} action notification
                  {wave18Unread === 1 ? "" : "s"}
                </span>
