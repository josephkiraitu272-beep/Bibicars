...
const RingostatManager = ({ prefs = null } = {}) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeIncoming = subscribe(
      "ringostat:incoming_call",
      handleIncomingCall,
    );
    const unsubscribeNeedsOutcome = subscribe(
      "ringostat:call_needs_outcome",
      handleCallNeedsOutcome,
    );
    const unsubscribeMissed = subscribe(
      "ringostat:missed_call",
      handleMissedCall,
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-base">
              {t("cmp_second_incoming_call")}
            </div>
            <div className="text-sm font-medium">
              {data.lead_name || data.from}
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              {t("cmp_first_call_is_still_active")}
            </div>
          </div>
        </div>,
        {
  ...
  const handleSaveOutcome = async () => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/manager/calls/${outcomeCall.call_id}/outcome`,
        {
          method: "POST",
          headers: {
  ...
  const loadCallsWithoutOutcome = async () => {
    try {
      ...
      if (data.calls) {
        ...
        const needsOutcome = data.calls.filter(
          (call) =>
            !call.outcome && call.duration > 30 && call.status === "ANSWERED",
        );
              ...
              {incomingCall.temperature !== undefined && (
                <Card>
                  ...
                  <CardContent>
                    <div className="flex items-center gap-2">
                      <Thermometer
                        className={`h-4 w-4 ${getTemperatureColor(incomingCall.temperature)}`}
                      />
                      <span
                        className={`font-semibold ${getTemperatureColor(incomingCall.temperature)}`}
                      >
                        {getTemperatureLabel(incomingCall.temperature)}
                      </span>
                      <span className="text-sm text-muted-foreground">
                        ({incomingCall.temperature}/100)
                      </span>
                    </div>
                  </CardContent>
                </Card>
      ...
      <Sheet open={showOutcomePanel} onOpenChange={setShowOutcomePanel}>
        <SheetContent side="right" className="w-[400px] sm:w-[540px]">
          <SheetHeader>
            ...
            <SheetDescription>
              {t("cmp_specify_the_result_of_the_conversation_with_the_cu")}
            </SheetDescription>
          </SheetHeader>
            ...
            <div className="mt-6 space-y-4">
                ...
                <Select value={outcome} onValueChange={setOutcome}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="no_answer">
                      {t("cmp_did_not_answer")}
                    </SelectItem>
                    <SelectItem value="vin_request">
                      {t("cmp_vin_request")}
                    </SelectItem>
                    <SelectItem value="delivery_discussion">
                      {t("cmp_delivery_discussion")}
                    </SelectItem>
                    <SelectItem value="ready_deposit">
                      {t("cmp_ready_for_deposit")}
                    </SelectItem>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t("cmp_ctrlenter_for_quick_save")}
                </p>
              </div>
