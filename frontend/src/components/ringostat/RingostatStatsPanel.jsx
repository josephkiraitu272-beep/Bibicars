...
const KPI = ({ label, value, hint, accent, icon: Icon, testid }) => (
  ...
  >
    <div className="flex items-start justify-between mb-1">
      ...
      {Icon ? (
        <Icon size={14} weight="duotone" className="text-[#A1A1AA]" />
      ) : null}
    </div>
    <p
...
const RingostatStatsPanel = () => {
  ...
  const maxDay = Math.max(
    1,
    ...(overview?.by_day || []).map((d) => d.total || 0),
  );
    ...
    <div className="space-y-5" data-testid="ringostat-stats-panel">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          ...
          icon={ArrowUpRight}
          accent={
            (totals.missed || 0) > (totals.answered || 0) ? "bad" : "warn"
          }
          hint={
            totals.missed || 0 ? t("rs_kpi_needs_callback") : t("rs_kpi_none")
          }
          testid="kpi-missed"
        />
        <KPI
            ...
            <table className="w-full text-[12px] tabular-nums">
              <thead className="text-[10.5px] uppercase tracking-wider text-[#71717A]">
                <tr className="border-b border-[#F4F4F5]">
                  ...
                  <th className="text-left font-semibold px-2 py-2 hidden md:table-cell">
                    {t("rs_th_last_call")}
                  </th>
                </tr>
              </thead>
                ...
                {managers.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`px-2 py-2 text-right font-semibold ${rateColor}`}
                      >
                        {rate.toFixed(1)}%
                      </td>
