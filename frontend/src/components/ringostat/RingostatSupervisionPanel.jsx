...
export default function RingostatSupervisionPanel({ role = "" }) {
  ...
  const isCompany = ["admin", "owner", "master_admin"].includes(
    (role || "").toLowerCase(),
  );
  ...
  if (collapsed) {
    return (
      <button
        ...
        title={t(
          isCompany ? "rs_company_calls_overview" : "rs_team_calls_overview",
        )}
  ...
  return (
    <>
      <Card
        className="fixed bottom-4 right-4 z-40 w-[320px] shadow-2xl border-slate-200 dark:border-slate-700"
        data-testid="ringostat-supervision-panel"
      >
        <CardContent className="p-3 space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm font-semibold">
              ...
              <Badge variant="outline" className="text-[10px]">
                {period === 1
                  ? t("rs_today")
                  : tn(period === 7 ? "rs_7d" : "rs_last_days_tpl", {
                      n: period,
                    })}
              </Badge>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <div>
              <div
                className={`text-xl font-bold ${kpi.missed > 5 ? "text-red-600" : "text-slate-500"}`}
              >
                {kpi.missed ?? 0}
              </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-center pt-1 border-t border-slate-100 dark:border-slate-800">
            ...
            <div>
              <div
                className={`text-lg font-semibold ${kpi.pending_outcome > 10 ? "text-amber-600" : "text-slate-500"}`}
              >
            ...
            <div>
              <div
                className={`text-lg font-semibold ${kpi.unassigned > 5 ? "text-red-600" : "text-slate-500"}`}
              >
                {kpi.unassigned ?? 0}
              </div>
              <div className="text-[10px] text-slate-500">
          ...
          {hasAlerts && (
            <div className="flex items-start gap-1.5 p-2 bg-amber-50 dark:bg-amber-950/30 rounded text-[11px] text-amber-800 dark:text-amber-300">
              ...
              <div>
                {alerts.many_missed && (
                  <div>
                    • {t("rs_too_many_missed")} ({kpi.missed})
                  </div>
                )}
                {alerts.many_pending_outcome && (
                  <div>
                    • {kpi.pending_outcome} {t("rs_calls_waiting_outcomes")}
                  </div>
                )}
                {alerts.many_unassigned && (
                  <div>
                    • {kpi.unassigned} {t("rs_unassigned_check_sip")}
                  </div>
                )}
              </div>
            </div>
          ...
          >
            {t(
              isCompany
                ? "rs_view_company_breakdown"
                : "rs_view_team_breakdown",
            )}
          </Button>
        </CardContent>
      </Card>
        ...
        <SheetContent className="overflow-y-auto sm:max-w-md">
          <SheetHeader>
            <SheetTitle>
              {t(
                isCompany
                  ? "rs_company_calls_overview"
                  : "rs_team_calls_overview",
              )}
            </SheetTitle>
            <SheetDescription>
              {period === 1
                ? t("rs_last_24h")
                : tn("rs_last_days_tpl", { n: period })}{" "}
              ·{" "}
              {overview.scope === "company"
                ? t("rs_all_managers")
                : tn("rs_members_tpl", { n: overview.team_size || 0 })}
            </SheetDescription>
          </SheetHeader>
            ...
            {managers.map((m) => (
              ...
              >
                ...
                <div className="grid grid-cols-4 gap-1 mt-1 text-center text-xs">
                  ...
                  <div>
                    <div
                      className={`font-semibold ${m.missed > 0 ? "text-red-600" : ""}`}
                    >
                      {m.missed}
                    </div>
                    <div className="text-[10px] text-slate-500">
                  ...
                  <div>
                    <div
                      className={`font-semibold ${m.pending_outcome > 0 ? "text-amber-600" : ""}`}
                    >
                      {m.pending_outcome}
                    </div>
                    <div className="text-[10px] text-slate-500">
