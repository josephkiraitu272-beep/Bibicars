  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await axios.get(
        `${API_URL}/api/customer-cabinet/${customerId}/orders`,
      );
  ...
  return (
    <div className="space-y-4">
      {orders.map((o) => {
        ...
        const doneSteps = (o.steps || []).filter(
          (s) => s.status === "done",
        ).length;
        ...
        for (const s of o.steps || []) {
          ...
          if (!groups[key])
            groups[key] = {
              name: s.service_name || t("adm3_108ede1c95"),
              steps: [],
            };
            ...
            <div className="bg-gradient-to-r from-[#635BFF]/5 to-[#9D8EFF]/5 px-5 py-4 border-b border-gray-100">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-medium mt-1 ${STATUS_BADGE[o.status] || "bg-gray-100 text-gray-600"}`}
                  >
              ...
              <div className="mt-3">
                ...
                <div className="h-2 bg-white border border-gray-100 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-[#635BFF] to-[#9D8EFF] rounded-full transition-all"
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
            ...
            <div className="p-5 space-y-4">
                    ...
                    <div className="space-y-1.5 pl-1">
                      {g.steps.map((s) => {
                        ...
                        return (
                          ...
                          >
                            <Icon
                              className={`w-4 h-4 ${meta.color} shrink-0`}
                            />
                            <span
                              className={`text-sm ${s.status === "done" ? "text-gray-500 line-through" : s.status === "in_progress" ? "text-gray-900 font-medium" : "text-gray-600"}`}
                            >
                              {s.label}
                            </span>
                            {s.note && (
              ...
              {o.invoiceId && (
                <div className="pt-3 border-t border-gray-100 flex items-center gap-1.5 text-xs text-gray-500">
                  <Receipt className="w-3.5 h-3.5" /> {t("adm3_3b5a72d11b")}{" "}
