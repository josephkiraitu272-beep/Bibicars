...
const RoadmapTab = ({ customerId }) => {
  ...
  const handleDelete = async (roadmapId) => {
    if (
      !window.confirm(
        "Cancel this roadmap? (soft-delete — it stays in history)",
      )
    )
      return;
    try {
      await axios.delete(`${API_URL}/api/roadmaps/${roadmapId}`, {
        headers: authHeaders(),
      });
  ...
  if (loading) {
    return (
      <div
        ...
        data-testid="roadmap-tab-loading"
      >
    ...
    <div className="space-y-4" data-testid="customer360-roadmap-tab">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-[#18181B]">
            {lang === "uk"
              ? "Дорожня карта клієнта"
              : lang === "bg"
                ? "Пътна карта на клиента"
                : "Client Roadmap"}
          </h3>
          <p className="text-sm text-[#71717A]">
            {pipelineType === "sales_pipeline"
              ? lang === "uk"
                ? "Воронка продажу: від нового ліда до завершеної угоди"
                : lang === "bg"
                  ? "Фуния на продажбата: от нов лийд до завършена сделка"
                  : "Sales pipeline: from new lead to completed deal"
              : lang === "uk"
                ? "Подорож авто: від аукціону до передачі клієнту"
                : lang === "bg"
                  ? "Пътят на автомобила: от търга до предаване"
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div
            ...
            data-testid="roadmap-type-switcher"
          >
            <button
      ...
      {roadmaps.length === 0 && (
        ...
        >
          ...
          <p className="text-[#71717A]">
            {pipelineType === "sales_pipeline"
              ? lang === "uk"
                ? "Дорожньої карти продажу немає. Натисніть «Нова», щоб створити."
                : lang === "bg"
                  ? "Няма пътна карта на продажбата. Натиснете «Нова», за да създадете."
                  : 'No sales pipeline roadmap yet. Click "New" to create one.'
              : lang === "uk"
                ? "Дорожніх карт ще немає. Нова створюється автоматично після оплати інвойсу."
                : lang === "bg"
                  ? "Няма пътни карти. Нова се създава автоматично след плащане на фактура."
        ...
        roadmaps.map((rm) => {
          ...
          return (
            <div
              ...
              data-testid={`roadmap-card-${rm.id}`}
            >
                  ...
                  <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-3">
                    {(rm.stages || [])
                      ...
                      .map((s) => (
                        <div
                          key={s.key}
                          className="p-3 rounded-xl border border-zinc-200 bg-zinc-50/40"
                        >
      ...
      {editing && (
        <div
          ...
          data-testid="roadmap-edit-modal"
        >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditing({ ...editing, sla_days: e.target.value })
                    }
                    className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditing({ ...editing, eta: e.target.value })
                    }
                    className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm"
                  />
                </div>
...
const NewRoadmapModal = ({
  ...
  const defaultInitial =
    (stageTemplate && stageTemplate[0]?.key) ||
    (pipelineType === "sales_pipeline" ? "new_lead" : "vehicle_found");
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1">
              {lang === "uk" ? "Назва" : lang === "bg" ? "Заглавие" : "Title"}
            </label>
            <input
              type="text"
              value={title}
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button
            onClick={submit}
            disabled={saving}
            className="px-4 py-2 bg-[#18181B] text-white text-sm rounded-lg hover:bg-[#3F3F46] disabled:opacity-50"
