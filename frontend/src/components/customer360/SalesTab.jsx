...
const StatusPill = ({ status, t }) => {
  const meta =
    STATUS_META[(status || "draft").toLowerCase()] || STATUS_META.draft;
...
const inputCls =
  "w-full h-9 px-3 rounded-lg border border-zinc-200 bg-white text-sm text-zinc-900 focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20 focus:outline-none transition-colors";
...
const FilesPickerModal = ({
  ...
  return (
    <div
      ...
      data-testid="sales-files-picker"
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-hidden flex flex-col">
        <div className="px-5 py-3.5 border-b flex items-center justify-between">
          <h3 className="text-base font-semibold text-zinc-900">
            {t("sales_docs") || "Documents"}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            <div className="text-sm text-zinc-400 text-center py-6">
              {t("sales_loading") || "Loading…"}
            </div>
          ) : files.length === 0 ? (
            <div className="text-sm text-zinc-400 text-center py-6">
              {t("sales_no_docs") || "No documents"}
            </div>
          ) : (
...
const SaleDialog = ({ open, onClose, customerId, sale, onSaved, t }) => {
  ...
  return (
    <div
      ...
      data-testid="sales-dialog"
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
        <div className="px-5 py-3.5 border-b flex items-center justify-between">
          <h3 className="text-base font-semibold text-zinc-900">
            {isEdit
              ? t("sales_edit") || "Edit Sale"
              : t("sales_add") || "Add Sale"}
          </h3>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-zinc-100"
        ...
        <div className="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Field label={t("sales_year") || "Year"}>
            <input
              ...
              className={inputCls}
              data-testid="sales-field-year"
            />
          </Field>
          <Field label={t("sales_country") || "Country"}>
            <select
              value={form.country || "OTHER"}
              onChange={(e) => setForm({ ...form, country: e.target.value })}
              className={inputCls}
              data-testid="sales-field-country"
            >
              {COUNTRIES.map((c) => (
          ...
          <Field label={t("sales_lot") || "Lot"}>
            <input
              ...
              className={inputCls}
              data-testid="sales-field-lot"
            />
          </Field>
          <Field label={t("sales_auction") || "Auction"}>
            <select
              value={form.auction || ""}
              onChange={(e) => setForm({ ...form, auction: e.target.value })}
              className={inputCls}
              data-testid="sales-field-auction"
            >
              <option value="">—</option>
              {AUCTIONS.map((a) => (
          ...
          <Field label={`${t("sales_amount") || "Amount"} *`}>
            <div className="flex gap-2">
              <input
                ...
                className={inputCls}
                data-testid="sales-field-amount"
              />
              <select
                value={form.saleCurrency || "USD"}
                onChange={(e) =>
                  setForm({ ...form, saleCurrency: e.target.value })
                }
                className={inputCls + " w-24"}
                data-testid="sales-field-currency"
              >
                {CURRENCIES.map((c) => (
                  <option key={c} value={c}>
                    {c}
          ...
          <Field label={t("sales_manager") || "Manager"}>
            <select
              ...
              className={inputCls}
              data-testid="sales-field-manager"
            >
              <option value="">
          ...
          <Field label={t("sales_contract") || "Contract"}>
            <select
              ...
              className={inputCls}
              data-testid="sales-field-contract"
            >
              <option value="">
          ...
          <Field label={t("sales_docs") || "Docs"}>
            <button
              ...
              className={
                inputCls +
                " flex items-center justify-between text-left hover:border-[#4F46E5]"
              }
          ...
          <Field label={t("sales_status") || "Status"}>
            <select
              ...
              className={inputCls}
              data-testid="sales-field-status"
            >
              <option value="draft">
                {t("sales_status_draft") || "Draft"}
              </option>
              <option value="active">
                {t("sales_status_active") || "Active"}
              </option>
              <option value="sold">{t("sales_status_sold") || "Sold"}</option>
              <option value="cancelled">
                {t("sales_status_cancelled") || "Cancelled"}
              </option>
            </select>
          </Field>
...
export default function SalesTab({ customerId }) {
  ...
  const cancelSale = async (s) => {
    if (!window.confirm(t("sales_confirm_delete") || "Cancel this sale?"))
      return;
    try {
  ...
  return (
    <div className="space-y-4" data-testid="customer360-sales-tab">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiCard
          label={t("sales_kpi_total") || "Total"}
          value={summary.total ?? 0}
          accent="#2563EB"
          testId="sales-kpi-total"
        />
        <KpiCard
          label={t("sales_kpi_sold") || "Sold"}
          value={summary.sold ?? 0}
          accent="#059669"
          testId="sales-kpi-sold"
        />
        <KpiCard
          label={t("sales_kpi_active") || "Active"}
          value={summary.active ?? 0}
          accent="#D97706"
          testId="sales-kpi-active"
        />
        <KpiCard
          label={t("sales_kpi_amount") || "Amount"}
          value={fmtMoney(summary.totalAmount || 0, fmCurrency)}
          accent="#18181B"
          testId="sales-kpi-amount"
        />
      </div>
            ...
            <table className="w-full text-sm min-w-[1200px]">
              <thead className="bg-zinc-50 text-zinc-600 text-[11.5px] uppercase">
                <tr>
                  ...
                  <th className="text-left px-3 py-2.5 font-semibold">
                    {t("sales_status") || "Status"}
                  </th>
                  <th className="text-left px-3 py-2.5 font-semibold">
                    {t("sales_comment") || "Comment"}
                  </th>
                  <th className="text-left px-3 py-2.5 font-semibold">
                    {t("dep7_col_utm")}
                  </th>
                ...
                {items.map((s) => (
                  <tr key={s.id} data-testid={`sales-row-${s.id}`}>
                    <td className="px-3 py-3 text-zinc-700 whitespace-nowrap">
                      {fmtDate(s.saleDate || s.created_at)}
                    </td>
                    <td className="px-3 py-3 text-zinc-900 font-medium whitespace-nowrap">
                      {s.customerName || "—"}
                    </td>
                    <td className="px-3 py-3 text-zinc-700 whitespace-nowrap font-mono text-[12px]">
                    ...
                    <td
                      className="px-3 py-3 text-zinc-600 max-w-[200px] truncate"
                      title={s.comment || ""}
                    >
                      {s.comment || "—"}
                    </td>
                      ...
                      (s.utm.utm_source ||
                        ...
                        >
                          {[
                            ...
                            s.utm.utm_campaign ? `· ${s.utm.utm_campaign}` : "",
                          ]
                            .filter(Boolean)
                            .join(" ")}
                        </span>
