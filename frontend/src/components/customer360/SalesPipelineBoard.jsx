...
const STATUS_THEME = {
  pending: {
    ...
    text: "text-zinc-600",
    label: { uk: "Очікує", en: "Pending", bg: "Изчаква" },
  },
  in_progress: {
    bg: "bg-amber-50",
    border: "border-amber-300",
    text: "text-amber-700",
    label: { uk: "В роботі", en: "In progress", bg: "В процес" },
  },
  done: {
    bg: "bg-emerald-50",
    border: "border-emerald-300",
    text: "text-emerald-700",
    label: { uk: "Готово", en: "Done", bg: "Готово" },
  },
  blocked: {
    bg: "bg-rose-50",
    border: "border-rose-300",
    text: "text-rose-700",
    label: { uk: "Заблоковано", en: "Blocked", bg: "Блокирано" },
  },
  skipped: {
    bg: "bg-zinc-100",
    border: "border-zinc-300",
    text: "text-zinc-500",
    label: { uk: "Пропущено", en: "Skipped", bg: "Пропуснат" },
  },
};
...
const StageCard = ({ stage, roadmapId, lang, isCurrent, onChanged }) => {
  ...
  const setStatus = async (status) => {
    try {
      await axios.patch(
        `${API_URL}/api/roadmaps/${roadmapId}/stages/${stage.key}`,
        { status },
      );
  ...
  const addRisk = async () => {
    ...
    try {
      await axios.post(
        `${API_URL}/api/roadmaps/${roadmapId}/stages/${stage.key}/risks`,
        newRisk,
      );
  ...
  const removeRisk = async (rid) => {
    try {
      await axios.delete(
        `${API_URL}/api/roadmaps/${roadmapId}/stages/${stage.key}/risks/${rid}`,
      );
      onChanged && onChanged();
    } catch (e) {
      toast.error(e.response?.data?.detail || "Failed");
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-zinc-200/70 bg-white/40">
        ...
        {stage.deadline_at ? (
          <div
            className={`mt-2 inline-flex items-center gap-1 text-[10px] ${stage.sla_breached ? "text-rose-700" : "text-zinc-500"}`}
          >
            <Clock className="w-3 h-3" />
            SLA {stage.sla_days || 0}d{stage.sla_breached ? " · overdue" : ""}
          </div>
        ) : null}
      </div>
        ...
        {(stage.checklist || []).length === 0 ? (
          ...
          (stage.checklist || []).map((c) => (
            ...
            >
              ...
              <span
                className={`text-[11.5px] ${c.done ? "line-through text-zinc-400" : "text-zinc-800"}`}
              >
                {c.label}
              </span>
            </button>
      ...
      {recommended && (
        <div className="px-4 pb-3">
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-2.5 py-1.5">
            ...
            <p className="text-[11.5px] text-blue-900 leading-snug">
              {recommended}
            </p>
          </div>
        </div>
      ...
      <div className="border-t border-zinc-200/70 bg-white">
        ...
        >
          <span>
            {lang === "uk" ? "Деталі" : lang === "bg" ? "Детайли" : "Details"}
          </span>
          {expanded ? (
          ...
          <div className="px-4 pb-3 space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                onBlur={() =>
                  comment !== (stage.comment || "") && saveComment()
                }
                rows={2}
                placeholder={
                  lang === "uk"
              ...
              />
              {savingComment && (
                <p className="text-[10px] text-zinc-400 mt-0.5">Saving…</p>
              )}
            </div>
            ...
            <div>
              ...
              {(stage.risks || []).map((r) => {
                ...
                return (
                  <div
                    ...
                    data-testid={`stage-risk-${r.id}`}
                  >
                    <span
                      className={`text-[10px] px-1.5 rounded ${sev.bg} ${sev.text} font-semibold`}
                    >
                      {r.severity}
                    </span>
                    <p className="text-[11.5px] text-zinc-700 flex-1 leading-snug">
                      {r.label}
                    </p>
                    <button
              ...
              <div className="flex gap-1 mt-1">
                <input
                  value={newRisk.label}
                  onChange={(e) =>
                    setNewRisk({ ...newRisk, label: e.target.value })
                  }
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewRisk({ ...newRisk, severity: e.target.value })
                  }
              ...
              <ul className="space-y-0.5">
                {(stage.transitions || [])
                  ...
                  .map((tr, idx) => (
                    <li
                      key={idx}
                      className="text-[10.5px] text-zinc-500 leading-snug"
                    >
                      <span className="font-mono">
                        {(tr.at || "").slice(5, 16).replace("T", " ")}
                      </span>
                      {" · "}
...
const SalesPipelineBoard = ({ roadmap, lang, onChanged }) => {
  ...
  return (
    <div
      ...
      data-testid="sales-pipeline-board"
    >
      <div className="flex items-center justify-between mb-3">
        <div>
          <h4 className="text-sm font-semibold text-zinc-900">
            {roadmap.title || "Sales Pipeline"}
          </h4>
          <p className="text-[11px] text-zinc-500">
            {lang === "uk" ? "Етап" : "Stage"} {Math.max(currentIdx + 1, 1)}/
            {stages.length}
            {" · "}
