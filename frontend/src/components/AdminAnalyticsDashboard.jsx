...
const KPICard = ({ title, value, icon, trend, color = "blue" }) => {
  ...
  return (
    <div className="min-w-0 kpi-card kpi-card--compact">
      <div className="mb-2 sm:mb-3">
        <span className={iconColors[color]}>{icon}</span>
      </div>
      <div className="truncate kpi-value kpi-value--fit" title={String(value)}>
        {value}
      </div>
      <div className="kpi-label">{title}</div>
      {trend && (
        <p
          className={`text-[11px] mt-1 ${trend >= 0 ? "text-emerald-600" : "text-red-600"}`}
        >
...
const FunnelChart = ({ data }) => {
  ...
  return (
    <div className="min-w-0 p-4 overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl sm:p-6">
      <h3 className="mb-3 text-base font-semibold sm:text-lg sm:mb-4">
        {t("i18n_conversion_funnel_417423")}
      </h3>
          ...
          return (
            <div key={step.name_key || step.name} className="relative">
              <div className="flex items-center justify-between mb-0.5 gap-2">
                <span className="text-[12.5px] sm:text-sm font-medium truncate">
                  {label}
                </span>
              ...
              <div className="h-2.5 sm:h-3 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className="h-full transition-all rounded-full bg-gradient-to-r from-blue-500 to-blue-400"
                  style={{
                    width: `${Math.max(2, (step.value / maxValue) * 100)}%`,
                  }}
                />
              </div>
              {idx < data.steps.length - 1 && (
                <div className="text-center text-[10px] leading-none text-gray-300 mt-0.5">
                  ↓
                </div>
              )}
            </div>
...
const SourcesTable = ({ data }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="min-w-0 p-4 overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl sm:p-6">
        <h3 className="mb-3 text-base font-semibold sm:text-lg sm:mb-4">
          {t("i18n_traffic_sources_f8f6bb")}
        </h3>
        <p className="text-sm text-gray-500">{t("i18n_no_data_ab3015")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-w-0 p-4 overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl sm:p-6">
      <h3 className="mb-3 text-base font-semibold sm:text-lg sm:mb-4">
        {t("i18n_traffic_sources_and_roi_0c15e6")}
      </h3>
      <div className="-mx-2 overflow-x-auto sm:mx-0">
        <table className="w-full min-w-[460px] text-[13px]">
          <thead>
            <tr className="border-b border-gray-200">
              <th className="text-left py-2 px-2 text-[11.5px] font-semibold text-gray-600 uppercase tracking-wide">
                {t("i18n_source_0945d8")}
              </th>
              <th className="text-right py-2 px-2 text-[11.5px] font-semibold text-gray-600 uppercase tracking-wide">
                {t("i18n_visits_e737a8")}
              </th>
              <th className="text-right py-2 px-2 text-[11.5px] font-semibold text-gray-600 uppercase tracking-wide">
                {t("i18n_leads_70641b")}
              </th>
...
const CampaignOptimizer = ({ data }) => {
  ...
  if (!data?.decisions || data.decisions.length === 0) {
    return (
      <div className="min-w-0 p-4 overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl sm:p-6">
        <h3 className="mb-3 text-base font-semibold sm:text-lg sm:mb-4">
          {t("i18n_campaign_optimizer_180c32")}
        </h3>
        ...
        <table className="w-full min-w-[680px] text-[13px]">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="text-center py-2 px-2 text-[11.5px] font-semibold text-gray-600 uppercase tracking-wide">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-semibold text-red-800">
            {t("i18n_fake_traffic_detected_04251b")}
          </p>
          <p className="text-sm text-red-600">
            {data.count} {t("i18n_suspicious_sessions_8bfbe5")} (
            {data.percentage}% {t("i18n_traffic_561df4")})
          </p>
        </div>
      </div>
    </div>
...
const AdminAnalyticsDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [dashboardRes, marketingRes] = await Promise.all([
        fetch(`${API_URL}/api/analytics/dashboard?${qs}`, { headers }).then(
          (r) => r.json(),
        ),
        fetch(`${API_URL}/api/analytics/marketing-campaigns?${qs}`, {
          headers,
        }).then((r) => r.json()),
      ]);
      ...
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="mt-3 text-gray-600">
            {t("i18n_loading_analytics_168307")}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-full min-w-0 space-y-4 overflow-x-hidden md:space-y-6">
      {/* Header */}
      <div className="flex flex-col min-w-0 gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-gray-500 sm:text-sm">
            {t("i18n_marketing_performance_and_roi_ab828b")}
          </p>
        </div>
        <div className="flex items-center w-full gap-2 sm:gap-3 sm:w-auto">
