    ...
    <div className="space-y-6" data-testid="admin-roadmaps-page">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900">
            {lang === 'bg' ? 'Пътни карти' : lang === 'uk' ? 'Дорожні карти' : 'Roadmaps'}
          </h1>
          <p className="text-sm text-zinc-500 mt-1">
            {pipelineType === 'sales_pipeline'
              ? (lang === 'uk' ? 'Воронка продажів по компанії' : lang === 'bg' ? 'Фуния на продажбите' : 'Sales pipeline across the company.')
              : (lang === 'uk' ? 'Прогрес усіх клієнтських авто від аукціону до передачі' : lang === 'bg' ? 'Прогрес на автомобилите от търга до предаване' : 'Vehicle progress from auction to handover, across the company.')}
          </p>
        </div>
      ...
      <div className="section-card">
        <h3 className="text-sm font-semibold text-zinc-700 mb-4">
          {lang === 'uk' ? 'Розподіл за етапами' : lang === 'bg' ? 'Разпределение по етапи' : 'Distribution by stage'}
        </h3>
      ...
      <div className="section-card">
        <h3 className="text-sm font-semibold text-zinc-700 mb-4">{lang === 'uk' ? 'Список' : lang === 'bg' ? 'Списък' : 'List'}</h3>
        {items.length === 0 ? (
