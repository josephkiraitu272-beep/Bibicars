...
const Staff = () => {
  ...
  const normalizeStaff = (u) => {
    ...
    const isActive =
      isActiveRaw === true || isActiveRaw === false ? isActiveRaw : !u.disabled;
    return {
      ...u,
  ...
  const fetchPerformance = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/api/staff/performance?period=${period}`,
      );
  ...
  const handleResetPassword = async (userId) => {
    ...
    try {
      await axios.post(`${API_URL}/api/staff/${userId}/reset-password`, {
        newPassword,
      });
      ...
      >
        ...
        <TabsContent value="list">
          <div className="card overflow-x-auto">
            <table
              className="table-premium min-w-[900px]"
                  ...
                  staff.map((user) => (
                    <tr key={user.id} data-testid={`staff-row-${user.id}`}>
                      ...
                      <td>
                        {user.teamId ? (
                          <span
                            className="inline-flex items-center px-2 py-0.5 rounded-md bg-[#E0E7FF] text-[#3730A3] text-xs font-medium"
                            data-testid={`staff-team-${user.id}`}
                          >
                            {user.teamId}
                          </span>
                        ) : (
                      ...
                      <td>
                        <span
                          className={`badge ${user.isActive ? "status-won" : "status-lost"}`}
                        >
        ...
        <TabsContent value="performance">
          ...
          <div className="flex flex-col gap-4 mb-6">
            ...
            <div
              className="period-tabs w-full sm:w-auto"
          ...
          {inactiveManagers.length > 0 && (
            <div
              className="bg-[#FEF3C7] border border-[#FCD34D] rounded-xl p-4 mb-6 flex items-start gap-3"
              data-testid="inactive-alert"
            >
          ...
          <div className="card overflow-x-auto">
            <table
              className="table-premium min-w-[800px]"
              ...
              <tbody>
                {performance.length === 0 ? (
                  ...
                  performance.map((manager) => (
                    <tr
                      key={manager.userId}
                      data-testid={`performance-row-${manager.userId}`}
                    >
                      <td>
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 min-w-[40px] bg-[#18181B] rounded-full flex items-center justify-center text-sm font-semibold text-white">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#71717A]">
                              {roleLabels[manager.userRole] || manager.userRole}
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="text-center">
                        <div className="flex items-center justify-center gap-2">
                          <span className="font-semibold text-[#18181B]">
                            {manager.leadsHandled}
                          </span>
                          <span className="text-xs text-[#059669]">
                            +{manager.leadsConverted}
                          </span>
                        </div>
                      </td>
                      <td className="text-center">
                        <span
                          className={`badge ${manager.conversionRate >= 30 ? "status-won" : manager.conversionRate >= 15 ? "status-contacted" : "status-lost"}`}
                        >
                      ...
                      <td className="text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {manager.tasksOverdue > 0 && (
                            <span className="text-xs text-[#DC2626]">
                              ({manager.tasksOverdue} {t("overdueCount")})
                            </span>
                          )}
                        </div>
                      </td>
                      <td className="text-center text-sm text-[#71717A]">
                        {manager.lastActivity
                          ? new Date(manager.lastActivity).toLocaleTimeString(
                              getLocale(),
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
                ...
                <span className="font-normal normal-case text-[#A1A1AA]">
                  (short code, e.g. sofia, korea — leave empty for none)
                </span>
              </label>
              <input
                type="text"
              ...
              <p className="mt-1.5 text-[11px] text-[#71717A]">
                Used by the reassignment ACL: a <b>Team Lead</b> can reassign
                workload only within their own team.
              </p>
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
