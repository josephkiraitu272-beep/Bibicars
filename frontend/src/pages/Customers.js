...
const Customers = () => {
      ...
      if (list.length) {
        try {
          const ids = list
            ...
            .filter(Boolean)
            .join(",");
          const { data: hb } = await axios.get(
            `${API_URL}/api/customer-health-bulk?ids=${ids}`,
          );
  ...
  const validate = () => {
    ...
    if (!(formData.firstName || "").trim())
      errors.firstName = t("field_required");
    if (!(formData.lastName || "").trim())
      errors.lastName = t("field_required");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCustomer) {
        ...
        if (inv?.invite_link) {
          if (inv.emailMode === "resend") {
            ...
            toast.message(
              "Invite link copied (dry-run — no email provider key yet)",
              {
      ...
      toast.error(
        err.response?.data?.detail || err.response?.data?.message || t("error"),
      );
    }
  };
  ...
  const openEditModal = (customer) => {
    ...
    setFormData({
      ...
      phoneCountry:
        customer.phoneCountry || (detected && detected.code) || "BG",
    ...
    >
      ...
      <div className="card p-4 sm:p-5 mb-4 sm:mb-5">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div
            ...
            data-testid="customers-health-filter"
          >
            {[
              { id: "", label: t("health_seg_all") },
              {
                id: "hot",
                label: t("health_seg_hot"),
                cls: "data-[active=true]:bg-emerald-50 data-[active=true]:text-emerald-700 data-[active=true]:border-emerald-300",
              },
              {
                id: "warm",
                label: t("health_seg_warm"),
                cls: "data-[active=true]:bg-amber-50   data-[active=true]:text-amber-700   data-[active=true]:border-amber-300",
              },
              {
                id: "cold",
                label: t("health_seg_cold"),
                cls: "data-[active=true]:bg-sky-50     data-[active=true]:text-sky-700     data-[active=true]:border-sky-300",
              },
              {
                id: "lost",
                label: t("health_seg_lost"),
                cls: "data-[active=true]:bg-zinc-100   data-[active=true]:text-zinc-700   data-[active=true]:border-zinc-300",
              },
            ].map((opt) => (
              <button
                key={opt.id || "all"}
        ...
        <div className="mt-3 sm:mt-4">
          ...
          <div
            ...
            data-testid="customers-filters-panel"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 sm:gap-3">
              ...
              {canReassign && (
                <div>
                  <label className="block text-[10px] uppercase tracking-wider text-[#71717A] mb-1">
                    {t("leadsWs_fieldManager") || "Manager"}
                  </label>
                  <select
                    value={managerFilter}
      ...
      <div className="card overflow-hidden">
        ...
        {canReassign && selectedIds.size > 0 && (
          <div
            className="bg-[#EEF2FF] border-b border-[#C7D2FE] px-4 py-2.5 flex items-center justify-between gap-3"
            data-testid="customers-bulk-bar"
          >
        ...
        <div className="hidden md:block overflow-x-auto">
          <table
            className="table-premium min-w-[700px] w-full"
            ...
            <tbody>
              {loading ? (
                ...
                customers
                  ...
                  .map((customer) => {
                    const mgr = customer.managerId
                      ? managersMap[customer.managerId]
                      : null;
                    ...
                    return (
                      <tr
                        ...
                        className={isSelected ? "bg-[#F5F3FF]" : ""}
                      >
                        {canReassign && (
                        ...
                        <td className="font-medium text-[#18181B]">
                          <button
                            onClick={() =>
                              navigate(`/admin/customers/${customer.id}/360`)
                            }
                        ...
                        <td>
                          {mgr ? (
                            <div className="flex items-center gap-1.5 text-xs">
                              ...
                              <span
                                className="text-[#18181B] truncate max-w-[110px]"
                                title={mgr.email}
                              >
                                {mgr.name || mgr.email}
                              </span>
                        ...
                        <td>
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <button
                              ...
                              className="p-2.5 hover:bg-[#E0E7FF] rounded-lg"
        ...
        <div
          className="md:hidden divide-y divide-[#F4F4F5]"
                  ...
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button
                        ...
                        className="p-2 hover:bg-[#E0E7FF] rounded-lg"
                        data-testid={`view-customer-mob-${customer.id}`}
                      >
                        <Eye size={16} className="text-[#4F46E5]" />
                      </button>
                      <button
                        ...
                        data-testid={`edit-customer-mob-${customer.id}`}
                      >
                        <Pencil size={16} className="text-[#71717A]" />
                      </button>
                      <button
                        ...
                        data-testid={`delete-customer-mob-${customer.id}`}
                      >
                        <Trash size={16} className="text-[#DC2626]" />
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-sm">
                    {customer.phone && (
                      <div>
                        ...
                        <div className="text-[#3F3F46] truncate">
                          {customer.phone}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="col-span-2 flex items-center justify-between pt-2 mt-1 border-t border-[#F4F4F5]">
                      ...
                      {customer.managerId && managersMap[customer.managerId] ? (
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-[#18181B] truncate max-w-[140px]">
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent
          className="max-w-[calc(100%-2rem)] sm:max-w-md bg-white rounded-2xl border border-[#E4E4E7] max-h-[90vh] overflow-y-auto"
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 mt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
                  {t("firstName")} <span className="text-[#DC2626]">*</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
                  {t("lastName")} <span className="text-[#DC2626]">*</span>
                </label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
                  {t("phone")} <span className="text-[#DC2626]">*</span>
                </label>
                <PhoneInput
                  value={formData.phone}
                  country={formData.phoneCountry}
                  onChange={({ phone, country }) =>
                    setFormData({ ...formData, phone, phoneCountry: country })
                  }
            ...
            {formData.type === "company" ? (
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
                  {t("company")} <span className="text-[#DC2626]">*</span>
                </label>
                <input
                  type="text"
            ...
            <div
              className="border border-[#E4E4E7] rounded-xl p-3 space-y-3 bg-[#FAFAFA]"
              data-testid="customer-wishes-block"
            >
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-[10.5px] uppercase tracking-wider text-[#71717A] mb-1">
                    {t("adm_budget_min")}
                  </label>
                  <input
                    type="number"
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...
                      wishes: {
                        ...formData.wishes,
                        timeline_months: e.target.value,
                      },
                    })
                  }
                  className="input w-full"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      wishes: { ...formData.wishes, note: e.target.value },
                    })
                  }
                  rows={2}
              ...
              >
                ...
                <span>
                  ...
                  <span className="block text-[12px] text-[#71717A] mt-0.5">
                    Email a 30-day link so the client sets their password and
                    signs in.
                  </span>
                </span>
              </label>
