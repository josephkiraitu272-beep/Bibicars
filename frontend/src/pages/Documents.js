...
const Documents = () => {
  ...
  useEffect(() => {
    fetchDocuments();
    if (isAdminOrFinance) fetchPendingDocs();
  ...
  const fetchPendingDocs = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/api/documents/queue/pending-verification`,
      );
      setPendingDocs(
  ...
  const handleReject = async () => {
    ...
    try {
      await axios.post(`${API_URL}/api/documents/${selectedDoc.id}/reject`, {
        reason: rejectReason,
      });
  ...
  const typeLabels = {
    ...
    deposit_proof: t("docDepositProof"),
    client_document: t("docClientDocument"),
    delivery_document: t("docDeliveryDocument"),
    custom: t("docOther"),
  };
  const statusLabels = {
    draft: t("docDraft"),
    uploaded: t("docUploaded"),
    pending_verification: t("docPendingVerification"),
    verified: t("docVerified"),
    rejected: t("docRejected"),
    archived: t("docArchived"),
  };
  const statusColors = {
    ...
    >
        ...
        {isAdminOrFinance && pendingDocs.length > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 bg-[#FEF3C7] border border-[#FCD34D] rounded-xl">
            ...
            <span className="text-sm font-medium text-[#92400E]">
              {pendingDocs.length} {t("pendingVerification")}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-5 mb-8">
        <div className="kpi-card" data-testid="docs-stat-total">
          <div className="mb-4">
            <FileText size={28} weight="duotone" className="text-[#4F46E5]" />
          </div>
        ...
        <div className="kpi-card" data-testid="docs-stat-pending">
          <div className="mb-4">
            <Clock size={28} weight="duotone" className="text-[#D97706]" />
          </div>
          <div className="kpi-value">
            {
              documents.filter((d) => d.status === "pending_verification")
                .length
            }
          </div>
          <div className="kpi-label">{t("pendingVerification")}</div>
        </div>
        <div className="kpi-card" data-testid="docs-stat-verified">
          ...
          <div className="kpi-label">{t("docVerified")}</div>
        </div>
        <div className="kpi-card" data-testid="docs-stat-rejected">
          <div className="mb-4">
            <XCircle size={28} weight="duotone" className="text-[#DC2626]" />
          </div>
          <div className="kpi-value">
            {documents.filter((d) => d.status === "rejected").length}
          </div>
          <div className="kpi-label">{t("docRejected")}</div>
        </div>
        <div className="kpi-card" data-testid="docs-stat-archived">
          <div className="mb-4">
            <Archive size={28} weight="duotone" className="text-[#71717A]" />
          </div>
          <div className="kpi-value">
            {documents.filter((d) => d.status === "archived").length}
          </div>
          <div className="kpi-label">{t("docArchived")}</div>
        </div>
      </div>
        ...
        >
          ...
          <TabsContent value="verification">
            <div className="section-card mb-6">
              <h3
                className="font-semibold text-[#18181B] mb-4"
                style={{
                  fontFamily:
                    "Mazzard, Mazzard H, Mazzard M, system-ui, sans-serif",
                }}
      ...
      <div className="flex flex-wrap gap-4">
        ...
        >
          <SelectTrigger
            ...
            data-testid="docs-status-filter"
          >
        ...
        >
          ...
          <tbody>
            {loading ? (
              <tr>
                <td
                  colSpan={isAdminOrFinance ? 5 : 4}
                  className="text-center !text-center align-middle py-12 text-[#71717A]"
                >
              ...
              <tr>
                <td
                  colSpan={isAdminOrFinance ? 5 : 4}
                  className="!text-center align-middle py-12 text-[#71717A]"
                >
                ...
                <tr key={doc.id} data-testid={`doc-row-${doc.id}`}>
                  ...
                  <td>
                    <span
                      ...
                      style={{
                        backgroundColor:
                          statusColors[doc.status]?.bg || "#F4F4F5",
                  ...
                  <td className="text-sm text-[#71717A]">
                    {doc.createdAt
                      ? new Date(doc.createdAt).toLocaleDateString(getLocale())
                      : "—"}
                  </td>
                  {isAdminOrFinance && (
                    <td>
