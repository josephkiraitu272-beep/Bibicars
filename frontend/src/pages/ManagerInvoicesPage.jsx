...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${m.color}`}
    >
...
function CreateInvoiceModal({ open, onClose, customers, services, onCreated }) {
  ...
  const submit = async () => {
    ...
    if (items.length === 0)
      return toast.error(t("cmp_add_at_least_one_service"));
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">
                {t("adm_selection_of_services_for_the_customer_multiline")}
              </p>
            </div>
          </div>
          <button
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-zinc-800 flex items-center gap-2">
                <ListChecks className="w-4 h-4 text-[#635BFF]" />{" "}
                {t("adm_invoice_items")}
            ...
            {items.length === 0 ? (
              <div className="text-center py-10 bg-zinc-50 rounded-2xl border border-dashed border-zinc-200">
                ...
                <p className="text-sm text-zinc-500">
                  {t("adm_no_positions_yet")}
                </p>
                <p className="text-xs text-zinc-400 mt-1">
                  {t("adm_click_add_service_to_select_from_the_catalog")}
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex-1 min-w-[220px]">
                      ...
                      {it.service_id ? (
                        <p className="text-[11px] text-zinc-400 mt-1 flex items-center gap-1">
                          <Package className="w-3 h-3" /> {t("r9_from_catalog")}{" "}
                          · {it.workflow.length} {t("r9_stages_many")}
                        </p>
                      ) : (
              ...
              <div className="flex-1 overflow-y-auto p-2">
                {filteredServices.length === 0 ? (
                  <div className="text-center py-8 text-sm text-zinc-500">
                    {t("r9_services_not_found")}. Master-admin{" "}
                    {t("r9_can_add_them_in")}{" "}
                    <code className="px-1.5 py-0.5 bg-zinc-100 rounded">
                      /admin/services
                    </code>
                    .
                  </div>
...
function InvoiceDetailDrawer({ invoice, onClose, onChanged }) {
  ...
  const act = async (action) => {
    ...
    try {
      ...
      toast.success(
        {
          send: t("adm3_01508f04ce"),
          cancel: t("adm3_4038ec22f7"),
          "mark-paid": t("adm3_0e4ba4e53d"),
        }[action] || "OK",
      );
        ...
        <div className="p-6 space-y-6">
          <div className="bg-gradient-to-br from-[#635BFF] to-[#7C6FFF] rounded-2xl p-5 text-white">
            ...
            <p className="text-3xl font-bold mt-1 tabular-nums">
              {fmt(invoice.total || invoice.amount, invoice.currency)}
            </p>
            <p className="text-xs opacity-80 mt-2">
              {t("adm3_e3fd7c1775")} {invoice.customerId}
            </p>
            {invoice.dueDate && (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-zinc-500">
                      ...
                      · {fmt(it.price, invoice.currency)} × {it.qty}
                    </p>
                  </div>
                  <span className="text-sm font-semibold tabular-nums">
                    {fmt(it.line_total, invoice.currency)}
                  </span>
                </div>
              ))}
            </div>
          ...
          {invoice.status === "paid" && (
            <div>
              ...
              {loadingOrder ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm font-medium text-emerald-900 mt-0.5">
                        {
                          (order.steps || []).filter((s) => s.status === "done")
                            .length
                        }
                        /{(order.steps || []).length} {t("r9_stages_completed")}
                      </p>
                    </div>
                ...
                <p className="text-sm text-zinc-500">
                  {t("adm_order_is_still_being_created")}
                </p>
              )}
            </div>
          )}
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [invR, svcR, cusR] = await Promise.all([
        axios.get(`${API_URL}/api/manager/invoices/my?limit=200`, {
          headers: authHeaders(),
        }),
        axios.get(`${API_URL}/api/services`),
        axios.get(`${API_URL}/api/customers?limit=500`, {
          headers: authHeaders(),
        }),
      ]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pending: invoices.filter((i) =>
        ["pending", "sent", "draft"].includes(i.status),
      ).length,
      paid: sumBy("paid"),
    ...
    <div className="space-y-6" data-testid="manager-invoices-page">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        {[
          ...
          {
            label: "Paid",
            value: fmt(stats.paid, "USD"),
            color: "#10B981",
            icon: CheckCircle2,
          },
          {
            label: "Overdue",
            value: fmt(stats.overdue, "USD"),
            color: "#EF4444",
            icon: AlertTriangle,
          },
        ].map((s, i) => {
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-wrap gap-1">
                      {(inv.items || []).slice(0, 3).map((it, i) => (
                        <span
                          key={i}
                          className="text-[11px] px-1.5 py-0.5 bg-zinc-100 rounded"
                        >
                          {it.name}
                        </span>
                      ))}
