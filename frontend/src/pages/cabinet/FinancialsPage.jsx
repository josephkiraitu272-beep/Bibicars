...
const PAYMENT_STATUS_META = {
  ...
  voided: {
    label: "Cancelled",
    tint: "bg-zinc-100 text-zinc-500",
    icon: IconX,
  },
};
...
export function CabinetFinancialsListPage() {
  ...
  return (
    <div
      ...
      data-testid="cabinet-financials-list"
    >
      <div className="mb-8">
        ...
        <p className="text-zinc-600">{t("adm3_93527eff64")}</p>
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${meta.tint}`}
                  >
                    {meta.label}
                  </span>
                </div>
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3 text-sm">
                    <div>
                      ...
                      <div className="font-mono font-bold text-zinc-900">
                        {fmt(summary.total_all)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase text-emerald-600 font-medium">
                        {t("adm3_079864b89e")}
                      </div>
...
export function CabinetDealFinancialsPage() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const r = await authedAxios().get(
        `/api/cabinet/deals/${dealId}/financials`,
      );
      setData(r.data);
    } catch (e) {
  ...
  const handlePay = async () => {
    setPaying(true);
    try {
      const r = await authedAxios().post(
        `/api/cabinet/deals/${dealId}/pay-intent`,
        {},
      );
  ...
  if (!data) {
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <button
          onClick={() => navigate("/cabinet/financials")}
  ...
  const officialDue = Math.max(
    0,
    (summary.total_official || 0) - (summary.paid_official || 0),
  );

  return (
    <div
      className="p-6 max-w-4xl mx-auto"
      data-testid="cabinet-deal-financials"
    >
      <button
        onClick={() => navigate("/cabinet/financials")}
        className="text-zinc-600 hover:text-zinc-900 mb-4 flex items-center gap-1 text-sm"
      ...
      <div className="bg-gradient-to-br from-emerald-50 to-blue-50 border border-emerald-200 rounded-2xl p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          ...
          <span
            className={`px-3 py-1.5 rounded-full text-sm font-semibold ${meta.tint}`}
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div>
            <div className="text-xs uppercase text-zinc-500 font-medium tracking-wider">
              {t("adm3_28807c941f")}
            </div>
            <div
              className="text-2xl font-mono font-bold text-zinc-900"
              data-testid="cabinet-total"
            >
              {fmt(total)}
            </div>
          </div>
          <div>
            <div className="text-xs uppercase text-emerald-600 font-medium tracking-wider">
              {t("adm3_079864b89e")}
            </div>
            <div
              className="text-2xl font-mono font-bold text-emerald-600"
              data-testid="cabinet-paid"
            >
              {fmt(paid)}
            </div>
          </div>
        ...
        {officialDue > 0 ? (
          ...
          >
            ...
            {paying
              ? t("adm3_f2e2c8113e")
              : `${t("r9_pay_amount_prefix")}${fmt(officialDue)}${t("r9_by_card_suffix")}`}
            <ArrowRight size={18} />
          </button>
        ) : remaining <= 0 && total > 0 ? (
        ...
        {payments.length === 0 ? (
          ...
          <div className="space-y-2">
            {payments.map((p) => {
              ...
              const stMeta =
                PAYMENT_STATUS_META[p.status] || PAYMENT_STATUS_META.pending;
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`px-2.5 py-1 rounded-md text-xs font-medium ${meth.tint}`}
                    >
                      {meth.label}
                    </div>
                    <div>
                      <div
                        className={`font-mono font-semibold ${isCash ? "text-red-700" : "text-zinc-900"}`}
                      >
                        {fmt(p.amount)}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${stMeta.tint}`}
                    >
        ...
        <tbody>
          {items.map((it, idx) => {
            ...
            return (
              <tr
                key={idx}
                className={`border-b border-zinc-100 ${isCash ? "bg-red-50/40" : ""}`}
              >
