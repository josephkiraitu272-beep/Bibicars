...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-sm font-medium border ${colors[status] || "bg-gray-100"}`}
    >
...
const DealBadge = ({ status }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-sm font-medium ${colors[status] || "bg-gray-400"}`}
    >
      {labels[status] || status}
    </span>
  );
...
const ConfidenceBar = ({ value }) => {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="w-12 text-sm font-medium">{percent}%</span>
    </div>
  );
};
...
const SourceItem = ({ source, expanded, onToggle }) => {
  ...
  return (
    <div className="overflow-hidden border border-gray-200 rounded-lg">
      <button
        onClick={onToggle}
        className="flex items-center justify-between w-full p-3 transition-colors hover:bg-gray-50"
      >
        <div className="flex items-center gap-3">
          {statusIcon}
        ...
        <div className="px-3 pt-0 pb-3">
          <div className="flex flex-wrap gap-2">
            {source.fieldsProvided.map((field) => (
              <span
                key={field}
                className="px-2 py-1 text-xs text-green-700 rounded bg-green-50"
              >
                {field}
              </span>
            ))}
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/api/vin-resolver/${vin.toUpperCase()}/test`,
      );
      setResult(res.data);
    } catch (err) {
    ...
    <div className="min-h-screen p-4 bg-gray-50 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="p-6 mb-6 bg-white border border-gray-200 shadow-sm rounded-2xl">
          <div className="flex gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full py-3 pl-12 pr-4 text-lg border border-gray-200 bg-gray-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                maxLength={17}
              />
            </div>
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-6 py-3 font-medium text-white transition-colors bg-blue-600 rounded-xl hover:bg-blue-700 disabled:opacity-50"
            >
              {loading ? (
          ...
          {error && (
            <div className="p-4 mt-4 text-red-700 border border-red-200 bg-red-50 rounded-xl">
              {error}
            </div>
          )}
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-6 bg-white border border-gray-200 shadow-sm rounded-2xl">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <div className="mb-1 text-sm text-gray-500">VIN</div>
            ...
            {v && (
              <div className="p-6 bg-white border border-gray-200 shadow-sm rounded-2xl">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {v.title && (
                    <div className="p-4 col-span-full bg-blue-50 rounded-xl">
                ...
                {v.images?.length > 0 && (
                  <div className="mt-6">
                    ...
                    <div className="flex gap-2 pb-2 overflow-x-auto">
                      ...
                      {v.images.length > 5 && (
                        <div className="flex items-center justify-center flex-shrink-0 w-32 h-24 text-gray-500 bg-gray-100 rounded-lg">
                          +{v.images.length - 5}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                    ...
                    {p.repairEstimate && (
                      <div>
                        ...
                        <span className="font-medium">
                          ${p.repairEstimate?.toLocaleString()}
                        </span>
                      </div>
                    )}
                    {p.platformMargin && (
                      <div>
                        ...
                        <span className="font-medium">
                          ${p.platformMargin?.toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
            ...
            <details className="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-2xl">
              <summary className="p-6 font-semibold cursor-pointer hover:bg-gray-50">
                {t("adm_raw_json_response")}
              </summary>
              <div className="p-6 pt-0">
                <pre className="p-4 overflow-x-auto text-xs text-green-400 bg-gray-900 rounded-xl">
