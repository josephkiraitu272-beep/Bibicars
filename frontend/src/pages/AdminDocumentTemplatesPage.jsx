...
const AdminDocumentTemplatesPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/admin/document-templates?${params}`,
        { headers: authHeaders() },
      );
  ...
  const seedDefaults = async () => {
    if (
      !window.confirm(
        "Створити шаблони за замовчуванням? Існуючі не будуть перезаписані.",
      )
    )
      return;
    try {
      await axios.post(
        `${API_URL}/api/admin/document-templates/seed-defaults`,
        {},
        { headers: authHeaders() },
      );
  ...
  const remove = async (tpl) => {
    ...
    try {
      await axios.delete(`${API_URL}/api/admin/document-templates/${tpl.id}`, {
        headers: authHeaders(),
      });
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            PDF шаблоны для договоров, инвойсов и актов приёма. Используют
            Jinja2 + HTML/CSS.
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-3 py-1.5 border border-zinc-200 rounded-lg text-sm hover:bg-zinc-50"
          >
            <ArrowsClockwise size={14} /> Seed defaults
          </button>
          <button
            onClick={() => setShowNew(true)}
            className="inline-flex items-center gap-1 px-3 py-1.5 bg-[#18181B] text-white rounded-lg text-sm hover:bg-[#27272A]"
            data-testid="doctpl-new-btn"
          >
        ...
        <div className="section-card max-h-[80vh] overflow-y-auto">
          {items.length === 0 ? (
            <p className="text-zinc-500 text-center py-8 text-sm">
              Нет шаблонов. Нажмите «Seed defaults».
            </p>
          ) : (
            grouped.map(([type, list]) => (
              <div key={type} className="mb-3">
                <p className="text-[10px] uppercase tracking-wider font-bold text-zinc-400 mb-1.5">
                  {type.replace(/_/g, " ")}
                </p>
                    ...
                    <li key={tpl.id}>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-[10px] uppercase tracking-wider font-mono ${selected?.id === tpl.id ? "text-zinc-300" : "text-zinc-500"}`}
                          >
                            {tpl.language}
                          </span>
                          {tpl.is_active ? (
            ...
            <div data-testid="doctpl-editor">
              <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 border border-red-200 text-red-600 hover:bg-red-50 rounded-lg text-sm inline-flex items-center gap-1"
                  >
                  ...
                  <button
                    onClick={save}
                    disabled={saving}
                    className="px-4 py-1.5 bg-[#18181B] text-white rounded-lg text-sm hover:bg-[#27272A] disabled:opacity-50 inline-flex items-center gap-1"
                    data-testid="doctpl-save"
                  >
              ...
              <div className="mb-3">
                <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1">
                  HTML body (Jinja2 + inline CSS)
                </label>
                <textarea
                  rows={22}
                  value={selected.html || ""}
                  onChange={(e) =>
                    setSelected({ ...selected, html: e.target.value })
                  }
                ...
                <p className="text-[11px] text-zinc-400 mt-1">
                  ...
                  {
                    '{{ customer.firstName }}, {{ invoice.total }}, {{ invoice["items"] }}, {{ company.name }}, {{ generated_at }}'
                  }
                  . Стили задаются через {"<style>"} / @page прямо в HTML.
                </p>
              </div>
            </div>
          )}
