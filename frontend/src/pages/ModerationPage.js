...
const ModerationPage = () => {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/publishing/queue?${params.toString()}`,
  ...
  const handleAction = async (id, action, extraData = {}) => {
    try {
      await axios.post(
        `${API_URL}/api/publishing/${id}/${action}`,
        { userId: user?.id || "admin", ...extraData },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const handleBulkAction = async (action) => {
    if (selectedIds.length === 0) {
      toast.warning(t("selectListings"));
      return;
    }
    try {
      await axios.post(
        `${API_URL}/api/publishing/bulk/${action}`,
        { ids: selectedIds, userId: user?.id || "admin" },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const toggleSelectAll = () => {
    ...
    else setSelectedIds(filteredListings.map((l) => l.id || l._id));
  };
        ...
        {Object.entries(statusConfig)
          ...
          .map(([status, config]) => (
            <div
              ...
              className={`p-4 rounded-xl border cursor-pointer transition-colors ${filter === status ? "border-zinc-900 bg-zinc-50" : "border-zinc-200 hover:border-zinc-300"}`}
            >
      ...
      <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-zinc-500">{t("noListings")}</p>
          </div>
        ) : (
      ...
      <td className="px-4 py-3">
        <span className="font-semibold text-zinc-900">
          {listing.currentBid || listing.buyNowPrice
            ? `$${(listing.currentBid || listing.buyNowPrice).toLocaleString()}`
            : "-"}
        </span>
      </td>
      <td className="px-4 py-3">
        <span
          className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${status.bg} ${status.color}`}
        >
          {status.label}
        </span>
      </td>
        ...
        <div className="flex items-center justify-end gap-1">
          {listing.status === "pending_review" && (
            <>
              ...
              <button
                onClick={() =>
                  onAction(listing.id || listing._id, "reject", {
                    reason: t("doesNotMeetRequirements"),
                  })
                }
