...
const L = {
  header:      { en: 'Top Deals of the Week — Builder', uk: 'Топ пропозиції тижня — Конструктор', bg: 'Топ оферти на седмицата — Конструктор' },
  desc:        { en: 'Curate the homepage wishlist. Submit cards by VIN → team-lead approves → public block updates.', uk: 'Кураторська добірка для головної. Додавайте картки за VIN → тимлід підтверджує → публічний блок оновлюється.', bg: 'Селекция за началната страница. Добавяйте карти по VIN → тийм-лийдът одобрява → публичният блок се обновява.' },
  ...
  empty:       { en: 'No wishlist cards yet — create your first pick above.', uk: 'Ще немає карток — створіть першу вище.', bg: 'Все още няма карти — създайте първата по-горе.' },
  del_confirm: { en: 'Delete this wishlist card?', uk: 'Видалити цю картку?', bg: 'Изтриване на тази карта?' },
  deleted:     { en: 'Deleted', uk: 'Видалено', bg: 'Изтрито' },
  del_failed:  { en: 'Delete failed', uk: 'Не вдалося видалити', bg: 'Неуспешно изтриване' },
  ...
  load_failed: { en: 'Failed to load wishlist', uk: 'Не вдалося завантажити', bg: 'Неуспешно зареждане' },
};
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          { id: 'pending', label: `${tr('pending')} (${grouped.pending.length})` },
          { id: 'approved', label: `${tr('approved')} (${grouped.approved.length})` },
          { id: 'rejected', label: `${tr('rejected')} (${grouped.rejected.length})` },
        ].map((f) => (
          <button
            key={f.id}
