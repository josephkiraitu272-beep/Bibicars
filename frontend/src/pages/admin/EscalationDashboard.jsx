...
const EVENT_LABELS = {
  "lead.hot_not_contacted": {
    uk: "HOT lead without contact",
    en: "HOT lead not contacted",
    bg: "HOT lead without contact",
  },
  "invoice.overdue": {
    uk: "Overdue bill",
    en: "Overdue invoice",
  ...
  "staff.session_suspicious": {
    uk: "Suspicious session",
    en: "Suspicious session",
    bg: "Suspicious session",
  },
};
...
export default function EscalationDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setEscalations(
        Array.isArray(escData) ? escData : escData.escalations || [],
      );
      setStats(statsData);
    } catch (error) {
  ...
  const getTimeRemaining = (deadline) => {
    ...
    if (hours > 0) {
      return {
        text: `${hours}${t("r9_h")} ${minutes % 60}${t("r9_min")}`,
        isOverdue: false,
      };
    }
  ...
  const totalActive =
    stats.managerPending + stats.teamLeadPending + stats.ownerPending;
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-xl">
        ...
        {escalations.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {escalations.map((esc) => {
              const levelConfig =
                LEVEL_CONFIG[esc.status] || LEVEL_CONFIG.manager_pending;
              const LevelIcon = levelConfig.icon;
              const deadline =
                esc.status === "manager_pending"
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div className={`p-2 rounded-lg ${levelConfig.bgLight}`}>
                        <LevelIcon
                          className={`w-5 h-5 ${levelConfig.textColor}`}
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-500">
                          <span>
                            {esc.entityType}: {esc.entityId?.slice(0, 8)}...
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs ${levelConfig.bgLight} ${levelConfig.textColor}`}
                          >
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`flex items-center gap-1 ${timeInfo.isOverdue ? "text-red-600 font-bold" : "text-gray-600"}`}
                      >
...
function EscalationModal({
  ...
  const { t } = useLang();
  const levelConfig =
    LEVEL_CONFIG[escalation.status] || LEVEL_CONFIG.manager_pending;
  const deadline =
    escalation.status === "manager_pending"
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg mx-4 overflow-hidden bg-white shadow-xl rounded-xl"
      ...
      >
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p
                className={`font-medium ${timeInfo.isOverdue ? "text-red-600" : "text-gray-900"}`}
              >
                {timeInfo.text}
              </p>
            </div>
          ...
          <div className="pt-4 border-t">
            ...
            <div className="space-y-2">
              ...
              <TimelineItem
                label={
                  lang === "uk" ? t("adm2_f9c9a5d595") : "Manager deadline"
                }
        ...
        <div className="flex gap-3 p-4 border-t bg-gray-50">
          ...
          <button
            onClick={onResolve}
            disabled={resolving}
            className="flex items-center justify-center flex-1 gap-2 px-4 py-2 text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-50"
          >
            {resolving ? (
