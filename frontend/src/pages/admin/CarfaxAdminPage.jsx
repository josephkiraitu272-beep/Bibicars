    ...
    >
      ...
      >
        <h3 className="text-lg font-bold mb-4">
          {t("adm3_pdf_0a095f9cae")} {request.vin}
        </h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">
              {t("adm_upload_pdf_file") || "Upload PDF file"}
            </label>
            <input
              type="file"
            ...
            />
            {pdfFile && (
              <p className="text-xs text-emerald-600 mt-1">{pdfFile.name}</p>
            )}
          </div>
...
const RejectModal = ({ request, onClose, onReject }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-lg font-bold mb-4">
          {t("adm3_506f69de2f")} {request.vin}
        </h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">
              {t("adm_reason_for_rejection")}
            </label>
            <textarea
              value={reason}
...
export default function CarfaxAdminPage() {
  ...
  const handleUpload = async (id, pdfUrl, actualCost) => {
    try {
      await axios.post(`${API_URL}/api/carfax/${id}/upload-pdf`, {
        pdfUrl,
        actualCost,
      });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-zinc-500">
              {t("adm_processing_report_requests")}
            </p>
          </div>
        </div>
        <button
                      ...
                      <p className="text-sm text-zinc-500">
                        {request.customerId ? (
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `${basePrefix}/customers/${request.customerId}/360?tab=carfax`,
                              )
                            }
                            className="text-blue-600 hover:underline font-medium"
                            data-testid={`carfax-open-customer-${request.id}`}
                            title={
                              t("adm_carfax_open_customer") ||
                              "Open customer card"
                            }
                          >
              ...
              >
                ...
                <div className="flex items-center gap-4 text-sm">
                  <span className="text-zinc-500">
                    {t("adm3_fdfbb7a2aa")} {m.processed}
                  </span>
                  <span className="text-emerald-600">
                    {t("adm3_627ce58368")} {m.uploaded}
                  </span>
                  <span className="text-red-600">
                    {t("adm3_ca877ce88a")} {m.rejected}
                  </span>
