...
const CHANNELS = {
  email: {
    labelKey: "emailLabel",
    icon: Mail,
    tipKey: "hub_tip_channel_email",
  },
  in_app: {
    labelKey: "inAppChannel",
    icon: Bell,
    tipKey: "hub_tip_channel_inapp",
  },
  sms: {
    labelKey: "smsChannel",
    icon: Smartphone,
    tipKey: "hub_tip_channel_sms",
  },
};
...
const TemplateDrawer = ({
  ...
  return createPortal(
    ...
    >
      ...
      <aside className="w-full max-w-3xl overflow-y-auto bg-white shadow-2xl">
        {/* Sticky header */}
        <div className="sticky top-0 z-10 flex flex-col justify-between gap-3 px-4 py-4 bg-white border-b border-zinc-200 sm:px-6 sm:flex-row sm:items-center">
          <div className="flex items-start min-w-0 gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs truncate text-zinc-500">
                ...
                {" · "}
                {LANGS.find((l) => l.code === draft.lang)?.flag}{" "}
        ...
        <div className="p-4 space-y-4 sm:p-6">
          ...
          <div>
            ...
            {preview ? (
              <div
                className="p-4 overflow-y-auto bg-white border rounded-lg border-zinc-200 max-h-96"
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                onChange({ ...draft, text_template: e.target.value })
              }
          ...
          <div className="p-3 text-xs border rounded-lg bg-zinc-50 border-zinc-100 text-zinc-500">
            <p className="mb-1 font-medium text-zinc-700">
              {t("adm_available_tokens")}
            </p>
...
export default function NotificationsHubPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [r, tpl] = await Promise.all([
        axios.get(`${API_URL}/api/admin/notification-rules`, {
          headers: authHeaders(),
        }),
        axios.get(`${API_URL}/api/admin/email-templates`, {
          headers: authHeaders(),
        }),
      ]);
  ...
  const saveRule = async (event, patch) => {
    try {
      ...
      setRules((prev) =>
        prev.map((x) => (x.event === event ? r.data.rule : x)),
      );
  ...
  const toggleChannel = (rule, audience, channel) => {
    const targets = (rule.targets || []).map((t0) => ({
      ...t0,
  ...
  const testDispatch = async (event) => {
    ...
    try {
      ...
      toast.success(
        `${t("r9_sent")} · ${t("r9_recipients_label")}: ${r.data?.dispatch?.total || 0}`,
      );
    } catch (e) {
      toast.error(e.response?.data?.detail || t("adm2_fd77287f02"));
    } finally {
  ...
  const findTemplate = useCallback(
    (event, audience, lang) =>
      templates.find(
        (tpl) =>
          tpl.event === event && tpl.audience === audience && tpl.lang === lang,
      ),
    [templates],
  );
  ...
  const testFromDrawer = async () => {
    ...
    try {
      ...
      toast.success(
        `Dispatch OK · ${r.data?.dispatch?.total || 0} ${t("recipients")}`,
      );
    } catch (e) {
  ...
  const filteredRules = useMemo(() => {
    ...
    return rules.filter((rule) => {
      ...
      if (filterAud) {
        const has = (rule.targets || []).some(
          (tg) => tg.audience === filterAud,
        );
        if (!has) return false;
      }
      if (filterChannel) {
      ...
      if (q) {
        const eventLabel = (
          EVENT_META[rule.event]?.fallback || humanizeEvent(rule.event)
        ).toLowerCase();
    ...
    <div className="space-y-6">
      ...
      <div className="mb-4 grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(220px,1fr))] sm:[grid-template-columns:minmax(280px,2fr)_repeat(3,minmax(180px,1fr))]">
        ...
        <WhiteSelect
          ...
          data-testid="notifications-hub-event-select"
        >
        ...
        <WhiteSelect
          ...
          data-testid="notifications-hub-audience-select"
        >
        ...
        <WhiteSelect
          ...
          data-testid="notifications-hub-channel-select"
        >
      ...
      <div className="space-y-4">
        {filteredRules.map((rule) => {
          const meta = EVENT_META[rule.event] || {
            icon: Bell,
            fallback: humanizeEvent(rule.event),
          };
          ...
          return (
            ...
            >
              ...
              <div className="px-5 sm:px-6 py-4 flex flex-wrap items-center justify-between gap-3 border-b border-[#E4E4E7] bg-zinc-50/40">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <HoverTip
                    text={
                      rule.enabled
                        ? t("hub_tip_disable_event")
                        : t("hub_tip_enable_event")
                    }
                    side="top"
                  >
                    <button
              ...
              <ul
                ...
                data-testid={`hub-audience-list-${rule.event}`}
              >
                {Object.entries(AUDIENCE)
                  ...
                  .map(([audKey, aud]) => {
                    ...
                    return (
                      <li
                        key={audKey}
                        className="flex flex-wrap items-center px-5 py-4 gap-x-6 gap-y-3 sm:px-6"
                          ...
                          {Object.entries(CHANNELS).map(([chKey, ch]) => {
                            ...
                            return (
                              ...
                              >
                                <button
                                  onClick={() =>
                                    toggleChannel(rule, audKey, chKey)
                                  }
                          ...
                          {LANGS.map((L) => {
                            ...
                            return (
                              ...
                              >
                                <button
                                  onClick={() =>
                                    openTemplate(rule.event, audKey, L.code)
                                  }
      ...
      <div className="mt-10">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <HoverTip text={t("hub_tip_integrations")} side="top">
              <h2 className="text-lg font-bold leading-tight text-gray-900 cursor-default sm:text-xl">
        ...
        <div className="bg-white border border-[#E4E4E7] rounded-2xl overflow-hidden">
          <button
            ...
            className="flex items-start w-full gap-3 px-5 py-4 text-left transition-colors sm:px-6 hover:bg-zinc-50/60"
          ...
          {advancedOpen ? (
            <div
              className="border-t border-[#E4E4E7] p-4 sm:p-5"
              data-testid="hub-advanced-panel"
            >
              <IntegrationsPage
                embedded
                filterProviders={["resend", "email", "sms"]}
              />
            </div>
          ) : null}
        </div>
