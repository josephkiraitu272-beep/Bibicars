...
const T = {
  uk: {
    ...
    pageSubtitle:
      "Тут налаштовується головна адреса сайту, копіюються посилання для зовнішніх сервісів (Ringostat, Stripe, Resend) і керується безпека сесій. Зміни застосовуються миттєво — рестарт не потрібен.",
    pageTooltip:
      "Єдиний майданчик для конфігурації публічного сайту та зовнішніх інтеграцій. Не плутайте з пунктом меню «System» — той блок керує внутрішніми параметрами CRM (модулі, права, кеші).",
    ...
    blockDomainHint:
      "Адреса, на якій працює CRM. Від неї будуються посилання в листах, договорах та інтеграціях.",
    blockDomainTip:
      "Це канонічна URL, яку отримують клієнти у листах, договорах і SMS. Якщо змінити домен — оновіть DNS-записи й SSL-сертифікат до збереження.",
    fieldMainAddress: "Основна адреса",
    fieldExample: "Приклад: https://bibi.cars (без слеша в кінці).",
    allowSubdomains: "Дозволити піддомени",
    allowSubdomainsHint:
      "Увімкніть, якщо у вас є admin.bibi.cars, app.bibi.cars чи інші піддомени.",
    // Block 2 — Integrations
    blockIntegrations: "Інтеграційні посилання",
    blockIntegrationsHint:
      "Скопіюйте ці URL у відповідні дашборди (Ringostat, Stripe, Resend), щоб сервіси відправляли події до CRM.",
    blockIntegrationsTip:
      "Це webhook-адреси, на які зовнішні сервіси будуть надсилати події (нові дзвінки, успішні оплати, статуси email). Без них Customer 360 не побачить активності з телефонії та платежів.",
    ...
    cardSiteOriginHint:
      "Канонічна URL — використовується в листах та договорах.",
    cardSiteTracker: "Скрипт для сайту",
    cardSiteTrackerHint:
      "Передайте цей фрагмент розробнику сайту bibi.cars — він вставить його перед закриваючим </body> на кожній сторінці. Завдяки цьому в Customer 360 і Lead 360 з\u2019являється вкладка «Активність».",
    ...
    blockSecurityHint:
      "Аварійна кнопка: миттєво вийти з усіх активних користувацьких сесій. Використовуйте лише за підозри на витік паролів.",
    blockSecurityTip:
      "Натискання генерує новий JWT-секрет на сервері: всі поточні токени стають недійсними, і кожен користувач муситиме увійти знову. Потрібен одноразовий рестарт backend.",
    securityWarningStrong: "Зверніть увагу:",
    securityWarning:
      "після натискання кнопки усі співробітники (і ви теж) автоматично вийдуть із системи. Знадобиться повторний вхід та одноразовий перезапуск сервера.",
    ...
    blockAdvancedHint:
      "Технічні параметри для devops-команди. Звичайному адміністратору не потрібні.",
    blockAdvancedTip:
      "CORS allowlist потрібен, якщо CRM API викликається з додаткових доменів (staging-середовища, preview-гілки, партнерські віджети). Помилка тут проявляється як «Cross-Origin» в консолі браузера.",
    advancedToggle: "CORS allowlist (для додаткових доменів)",
    advancedDescr:
      "Додаткові домени, з яких браузер може звертатись до CRM API: preview-гілки, staging-середовища, партнерські віджети тощо. Основний домен (вище) додається автоматично.",
  ...
  en: {
    ...
    pageSubtitle:
      "Configure the main site URL, copy integration endpoints (Ringostat, Stripe, Resend) and manage session security. Changes apply instantly — no restart required.",
    pageTooltip:
      "Single place to configure the public website and external integrations. Not to be confused with the «System» menu entry — that one manages internal CRM parameters (modules, permissions, caches).",
    btnSaving: "Saving…",
    ...
    blockDomainHint:
      "The address where the CRM lives. It powers the URLs used in emails, contracts and integrations.",
    blockDomainTip:
      "This is the canonical URL customers receive in emails, contracts and SMS. If you change it — update DNS records and the SSL certificate before saving.",
    fieldMainAddress: "Main address",
    fieldExample: "Example: https://bibi.cars (no trailing slash).",
    allowSubdomains: "Allow subdomains",
    allowSubdomainsHint:
      "Enable if you also use admin.bibi.cars, app.bibi.cars or other subdomains.",
    blockIntegrations: "Integration endpoints",
    blockIntegrationsHint:
      "Paste these URLs into the corresponding dashboards (Ringostat, Stripe, Resend) so they push events to the CRM.",
    blockIntegrationsTip:
      "These webhook URLs receive events from external services (new calls, successful payments, email delivery status). Without them, Customer 360 will not see telephony and payment activity.",
    ...
    cardSiteTrackerHint:
      "Hand this snippet to the bibi.cars web developer — it must be placed before the closing </body> on every page. After that, the «Activity» tab in Customer 360 and Lead 360 starts receiving events.",
    ...
    blockSecurityHint:
      "Emergency button: log every active user out instantly. Use only when a credential leak is suspected.",
    blockSecurityTip:
      "Pressing this generates a fresh JWT secret on the server: every existing token becomes invalid and every user must sign in again. A one-off backend restart is required.",
    securityWarningStrong: "Heads up:",
    securityWarning:
      "after the button is pressed every staff member (and you too) will be signed out automatically. A re-login and a one-off server restart will be required.",
    ...
    blockAdvancedHint:
      "Technical knobs for the DevOps team. Regular admins don\u2019t need to touch them.",
    blockAdvancedTip:
      "The CORS allowlist is required when the CRM API is called from additional domains (staging environments, preview branches, partner widgets). A misconfiguration surfaces as a «Cross-Origin» error in the browser console.",
    advancedToggle: "CORS allowlist (extra domains)",
    advancedDescr:
      "Extra domains from which the browser can call the CRM API: preview branches, staging environments, partner widgets, and so on. The main domain (above) is added automatically.",
  ...
  bg: {
    ...
    pageSubtitle:
      "Тук се конфигурира основният URL на сайта, копират се връзките за външни услуги (Ringostat, Stripe, Resend) и се управлява сигурността на сесиите. Промените се прилагат веднага — не е нужен рестарт.",
    pageTooltip:
      "Единно място за конфигуриране на публичния сайт и външните интеграции. Не бъркайте с менюто «System» — този раздел управлява вътрешни параметри на CRM (модули, права, кешове).",
    btnSaving: "Запазване…",
    ...
    blockDomainHint:
      "Адресът, на който работи CRM. От него се изграждат връзките в писма, договори и интеграции.",
    blockDomainTip:
      "Това е каноничният URL, който клиентите получават в писма, договори и SMS. Ако го промените — обновете DNS записите и SSL сертификата преди да запазите.",
    ...
    allowSubdomainsHint:
      "Включете, ако имате admin.bibi.cars, app.bibi.cars или други поддомейни.",
    blockIntegrations: "Интеграционни връзки",
    blockIntegrationsHint:
      "Поставете тези URL-и в съответните табла (Ringostat, Stripe, Resend), за да изпращат събития към CRM.",
    blockIntegrationsTip:
      "Това са webhook адреси, на които външните услуги ще изпращат събития (нови обаждания, успешни плащания, статуси на доставка на писма). Без тях Customer 360 няма да вижда активността от телефония и плащания.",
    ...
    cardSiteTrackerHint:
      "Предайте този фрагмент на разработчика на bibi.cars — той трябва да го постави преди затварящия </body> на всяка страница. След това разделът «Активност» в Customer 360 и Lead 360 започва да получава събития.",
    ...
    blockSecurityHint:
      "Аварийно копче: незабавно излизане от всички активни сесии. Използвайте само при съмнения за компрометирани пароли.",
    blockSecurityTip:
      "Натискането генерира нов JWT секрет на сървъра: всички текущи токени стават невалидни и всеки потребител ще трябва да влезе отново. Необходим е еднократен рестарт на backend.",
    securityWarningStrong: "Внимание:",
    securityWarning:
      "след натискане всички служители (вкл. вие) автоматично ще бъдат отписани. Ще е необходимо повторно влизане и еднократен рестарт на сървъра.",
    btnTerminateAll: "Прекрати всички активни сесии",
    confirmTerminate:
      "Прекратяване на всички активни сесии?\n\nВсички потребители (включително вие) ще бъдат отписани. Използвайте само при съмнения за изтичане на ключове.",
    toastTerminated:
      "Всички сесии са прекратени. Необходим е рестарт на backend.",
    blockAdvanced: "Разширени настройки",
    blockAdvancedHint:
      "Технически параметри за DevOps екипа. Не са нужни на обикновения администратор.",
    blockAdvancedTip:
      "CORS allowlist е необходим, когато CRM API се извиква от допълнителни домейни (staging среди, preview-разклонения, партньорски виджети). Грешка тук се проявява като «Cross-Origin» в конзолата на браузъра.",
    advancedToggle: "CORS allowlist (допълнителни домейни)",
    advancedDescr:
      "Допълнителни домейни, от които браузърът може да обръща към CRM API: preview-разклонения, staging среди, партньорски виджети и т.н. Основният домейн (по-горе) се добавя автоматично.",
  ...
  >
    <header className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {hint ? (
          <p className="text-[12.5px] text-[#71717A] mt-1 leading-relaxed break-words">
            {hint}
          </p>
        ) : null}
      </div>
    ...
    >
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-[13.5px] font-semibold text-[#18181B] leading-tight">
            {name}
          </h4>
          {description ? (
            <p className="text-[11.5px] text-[#71717A] mt-0.5 leading-snug break-words">
              {description}
            </p>
          ) : null}
        </div>
      ...
      <div className="flex items-stretch gap-2">
        <div className="flex-1 min-w-0 h-10 px-3 rounded-lg border border-[#E4E4E7] bg-[#FAFAFA] text-[12px] sm:text-[12.5px] text-[#52525B] font-mono flex items-center truncate">
          {url || (
            <span className="italic text-[#A1A1AA] truncate">
              {t("placeholderUrl")}
            </span>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="rounded-xl border border-[#E4E4E7] bg-zinc-950 overflow-hidden">
        ...
        <pre className="overflow-x-auto px-4 py-3 text-[11.5px] sm:text-[12px] leading-relaxed text-zinc-100 font-mono whitespace-pre">
          {snippet || `<!-- ${t("placeholderDomain")} -->`}
        </pre>
      </div>
    </div>
  );
...
const AdminSystemSettingsPage = () => {
  ...
  return (
    <div
      className="space-y-5 sm:space-y-6 w-full max-w-full overflow-hidden"
    ...
    >
      ...
      <Section
        icon={Browser}
        title={t("blockDomain")}
        hint={t("blockDomainHint")}
        tooltip={t("blockDomainTip")}
        testId="section-domain"
      >
        <TextField
        ...
        <label className="flex items-start gap-3 cursor-pointer select-none pt-1">
          <input
            ...
            onChange={(e) =>
              setDraft({ ...draft, allow_subdomains: e.target.checked })
            }
      ...
      <Section
        icon={Plug}
        title={t("blockIntegrations")}
        hint={t("blockIntegrationsHint")}
        tooltip={t("blockIntegrationsTip")}
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <IntegrationCard
            t={t}
            icon={PhoneCall}
            name={t("cardRingostat")}
            description={t("cardRingostatHint")}
            url={computed.ringostat_webhook_url}
            testid="ringostat"
          />
          <IntegrationCard
            t={t}
            icon={CreditCard}
            name={t("cardStripe")}
            description={t("cardStripeHint")}
            url={computed.stripe_webhook_url}
            testid="stripe"
          />
          <IntegrationCard
            t={t}
            icon={PaperPlaneTilt}
            name={t("cardResend")}
            description={t("cardResendHint")}
            url={computed.resend_webhook_url}
            testid="resend"
          />
          <IntegrationCard
            t={t}
            icon={Globe}
            name={t("cardSiteOrigin")}
            description={t("cardSiteOriginHint")}
            url={computed.site_origin}
            testid="site-origin"
          />
          <ScriptSnippetCard
            t={t}
            snippet={snippet}
            testid="site-tracker-snippet"
          />
        </div>
      </Section>
      ...
      <Section
        icon={ShieldCheck}
        title={t("blockSecurity")}
        hint={t("blockSecurityHint")}
        tooltip={t("blockSecurityTip")}
      ...
      {isMasterAdmin ? (
        <Section
          icon={Info}
          title={t("blockAdvanced")}
          hint={t("blockAdvancedHint")}
          tooltip={t("blockAdvancedTip")}
            ...
            <div className="space-y-3 pt-2" data-testid="advanced-content">
              ...
              <div className="space-y-2">
                {(draft.cors_origins || []).map((o) => (
                  ...
                  >
                    <span className="text-[12.5px] text-[#18181B] font-mono truncate">
                      {o}
                    </span>
                    <button
                      type="button"
      ...
      {data?.settings?.updated_at ? (
        <p className="text-[11.5px] text-[#A1A1AA] text-center break-words px-2">
          ...
          <span className="font-mono">
            {data.settings.updated_by || t("footerBy")}
          </span>
        </p>
      ) : null}
    </div>
