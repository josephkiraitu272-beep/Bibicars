...
const T = {
  uk: {
    ...
    page_subtitle:
      "Цей скрипт треба вставити на bibi.cars. Він автоматично пінгує CRM подіями (cabinet_login, form_active, form_submitted, callback_request, session_end) і відображає онлайн-статус ліда чи клієнта в Customer 360.",
    loading: "Завантаження…",
    section_install: "Установка (1 рядок коду)",
    section_install_hint:
      "Скопіюйте та вставте перед </body> на ВСІХ сторінках публічного сайту. Більше нічого робити не потрібно — решта подій спрацьовує автоматично.",
    section_identify: "Прив'язка відвідувача (опційно)",
    section_identify_hint:
      "Якщо відвідувач уже авторизований або заповнив форму, передайте phone/email — CRM зв'яже подію з конкретним лідом/клієнтом.",
    section_manual: "Ручні події",
    section_manual_hint:
      'Викликайте window.bibiTracker.track() щоразу, коли треба зафіксувати дію (напр. "Замовити дзвінок").',
    section_endpoints: "Кінцеві точки API",
    section_events: "Типи подій, що відстежуються",
    section_test: "Перевірка",
    section_test_hint:
      "Відправити тестову подію прямо звідси, щоб переконатися, що ingest працює.",
    ...
    api_key_hint:
      "Цей ключ вшитий у <script>. Сам по собі він не дає доступу до CRM — лише до ingest endpoint.",
  },
  en: {
    page_title: "JS-tracker for the public site",
    page_subtitle:
      "Paste this snippet on bibi.cars to make the CRM receive real-time activity events (cabinet_login, form_active, form_submitted, callback_request, session_end). Lead/Customer 360 will then show the live online-status badge.",
    loading: "Loading…",
    section_install: "Installation (one-line snippet)",
    section_install_hint:
      "Copy and paste before </body> on EVERY page of the public website. No further setup is required — the rest is automatic.",
    section_identify: "Bind visitor to a known person (optional)",
    section_identify_hint:
      "When the visitor is logged in or has filled the contact form, pass phone/email so the CRM can link the events to a specific lead/customer.",
    section_manual: "Manual events",
    section_manual_hint:
      'Call window.bibiTracker.track() whenever you need to record an action (e.g. "Request a call").',
    section_endpoints: "API endpoints",
    section_events: "Tracked event types",
    section_test: "Live test",
    section_test_hint:
      "Fire a test event right from here to verify the ingest pipeline.",
    ...
    api_key_hint:
      "This key is embedded in the <script> tag. By itself it grants ingest access only — no CRM data is exposed.",
  },
  bg: {
    page_title: "JS-тракер за външния сайт",
    page_subtitle:
      "Поставете този скрипт на bibi.cars. Той автоматично изпраща събития (cabinet_login, form_active, form_submitted, callback_request, session_end) към CRM и онлайн-статусът се появява в Customer 360.",
    loading: "Зареждане…",
    section_install: "Инсталация (един ред код)",
    section_install_hint:
      "Копирайте и поставете преди </body> на ВСИЧКИ страници от публичния сайт.",
    section_identify: "Свързване на посетител (опционално)",
    section_identify_hint:
      "Когато посетителят е логнат или вече е попълнил форма, подайте phone/email, за да свърже CRM събитията с конкретен клиент.",
    section_manual: "Ръчни събития",
    section_manual_hint:
      'Извикайте window.bibiTracker.track() при нужда (напр. "Поискай обаждане").',
    ...
    api_key_hint:
      "Този ключ е вграден в <script>. Сам по себе си дава достъп само до ingest endpoint.",
  },
};
...
const CodeBlock = ({ code, lang = "html", label, testId, onCopy, copied }) => (
  <div
    className="relative rounded-xl border border-zinc-200 bg-zinc-950 overflow-hidden"
...
export default function AdminSiteTrackerPage() {
  ...
  const sendTestEvent = useCallback(async () => {
    ...
    try {
      ...
      const matched = json.matched
        ? ` → matched ${json.target?.kind} ${json.target?.name}`
        : " (no match — visitor anonymous)";
  ...
  return (
    <div className="space-y-6" data-testid="admin-site-tracker-page">
      ...
      <Section
        icon={Code}
        title={`1. ${t("section_install")}`}
        subtitle={t("section_install_hint")}
      >
        <CodeBlock
          code={data.snippet}
          lang="HTML"
      ...
      <Section
        icon={ShieldCheck}
        title={`2. ${t("section_identify")}`}
        subtitle={t("section_identify_hint")}
      >
        <CodeBlock
      ...
      <Section
        icon={Lightning}
        title={`3. ${t("section_manual")}`}
        subtitle={t("section_manual_hint")}
      >
        <CodeBlock
      ...
      <Section icon={Globe} title={t("section_endpoints")}>
        <div className="grid sm:grid-cols-2 gap-3">
          <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3">
            <p className="text-[11px] uppercase tracking-wider font-bold text-zinc-500 mb-1">
              POST · ingest
            </p>
            <code className="text-[12px] font-mono text-zinc-800 break-all">
              {data.ingest_url}
            </code>
          </div>
          <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3">
        ...
        <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-amber-800">
              {t("api_key")} · {data.api_key_header}
            </p>
            <code
              className="text-[12px] font-mono text-amber-900 break-all"
      ...
      <Section
        icon={Lightning}
        title={t("section_test")}
        subtitle={t("section_test_hint")}
      >
        <div className="grid sm:grid-cols-[1fr_1fr_auto] gap-3 items-end">
          <div>
            <label
              htmlFor="tracker-test-phone"
              className="block text-[11px] uppercase tracking-wider font-bold text-zinc-500 mb-1"
            >
              {t("test_phone")}
            </label>
            <input
