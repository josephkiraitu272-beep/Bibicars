...
const SourceHealthDashboard = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case "active":
        return <CheckCircle weight="fill" className="text-emerald-500" />;
      case "degraded":
        return <Warning weight="fill" className="text-amber-500" />;
      case "quarantine":
        return <XCircle weight="fill" className="text-red-500" />;
      case "disabled":
        return <XCircle className="text-zinc-400" />;
      default:
        return null;
    }
  };
  ...
  const getTierBadge = (tier) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 text-xs text-white rounded ${colors[tier] || colors[4]}`}
      >
        Tier {tier}
      </span>
    );
                  ...
                  <tbody className="divide-y divide-zinc-100">
                    {data.sources.map((source) => (
                      <tr key={source.name} className="hover:bg-zinc-50">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`px-2 py-1 text-xs rounded-full font-medium ${getStatusColor(source.status)}`}
                          >
                            {source.status}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-sm ${source.hitRate > 0.5 ? "text-emerald-600" : "text-zinc-500"}`}
                          >
                        ...
                        <td className="px-4 py-3 text-sm text-zinc-600">
                          {source.avgLatency > 0
                            ? `${Math.round(source.avgLatency)}ms`
                            : "-"}
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-sm text-zinc-600">
