...
function Stat({ label, value, sub, icon: Icon, tone = "slate" }) {
  ...
  return (
    <div className="rounded-xl border border-[#E4E4E7] bg-white p-4">
      ...
      <div className="mt-1.5 text-[22px] font-bold text-[#18181B] leading-tight">
        {value ?? "—"}
      </div>
      {sub ? (
        <div className="mt-0.5 text-[12px] text-[#71717A] truncate">{sub}</div>
      ) : null}
    </div>
  );
}
...
export default function VesselFinderSessionPage() {
  ...
  const loadExtClients = useCallback(async () => {
    setExtError(null);
    try {
      const r = await axios.get(`${API_URL}/api/admin/ext-clients`, {
        headers: authHeaders(),
      });
  ...
  const loadSharedSecret = useCallback(async () => {
    try {
      const r = await axios.get(
        `${API_URL}/api/admin/ext-clients/shared-secret`,
        { headers: authHeaders() },
      );
    ...
    try {
      const r = await axios.post(
        `${API_URL}/api/admin/ext-clients/bootstrap`,
        {
          ...
          note: "Created from Vessel Sync admin page",
        },
        { headers: authHeaders() },
      );
  ...
  const rotateExtClient = async (clientId) => {
    if (
      !window.confirm(
        `Rotate secret for ${clientId}? The old secret stops working immediately.`,
      )
    )
      return;
    setExtLoading(true);
  ...
  const createTestTracking = async () => {
    ...
    try {
      const r = await axios.post(
        `${API_URL}/api/vesselfinder/session/test-tracking`,
      );
      toast.success(t("vfTestTrackingCreated"), {
        description: r.data?.shipmentId,
      });
      await loadStatus();
    } catch (e) {
      toast.error(t("vfTestTrackingFail"), {
  ...
  const removeTestTracking = async () => {
    ...
    try {
      ...
      toast.success(t("vfTestTrackingRemoved"), {
        description: `${r.data?.removed ?? 0}`,
      });
      await loadStatus();
    } catch (e) {
      toast.error(t("vfTestTrackingFail"), {
  ...
  const tickShipment = async (shipmentId) => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/shipments/${shipmentId}/tick`,
      );
      setTickResults((prev) => ({
        ...prev,
        [shipmentId]: { ok: true, data: res.data, at: new Date() },
      }));
    } catch (e) {
      setTickResults((prev) => ({
        ...prev,
  ...
  const tickAllActive = async () => {
    ...
    if (
      !window.confirm(
        t("vfTickActiveConfirm").replace("{count}", active.length),
      )
    )
      return;
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      const [dbRes, richRes, liveRes] = await Promise.allSettled([
        ...
        axios.get(`${API_URL}/api/admin/shipments/search`, {
          params: { q, limit: 30 },
        }),
        axios.get(`${API_URL}/api/vesselfinder/vessels/search`, {
          params: { bbox: "-180,-80,180,80", query: q },
        }),
      ]);
      setSearchData({
        db:
          dbRes.status === "fulfilled"
            ? dbRes.value.data
            : { error: String(dbRes.reason) },
        rich:
          richRes.status === "fulfilled"
            ? richRes.value.data
            : { error: String(richRes.reason) },
        live:
          liveRes.status === "fulfilled"
            ? liveRes.value.data
            : {
                error: String(
                  liveRes.reason?.response?.data?.detail || liveRes.reason,
                ),
              },
      });
    } finally {
  ...
  const loadVesselHistory = useCallback(async (sid) => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/api/shipments/${sid}/vessel-history`,
      );
    ...
    try {
      ...
      if (bindVin.trim() && !bindShipmentId) {
        ...
        if (!bindShipmentId) {
          setBindResult({
            ok: false,
            error: t("adm2_shipment_id_vin_43d326d0b8"),
          });
          return;
        }
        const res = await axios.post(
  ...
  const doTransferVessel = async () => {
    if (!bindShipmentId) {
      setBindResult({ ok: false, error: t("vfBindChooseShipment") });
      return;
    }
    ...
    const confirmMsg = t("vfBindConfirmMsg").replace(
      "{name}",
      bindName || bindMmsi,
    );
  ...
  const extensionOk =
    status?.heartbeatAgeSec != null && status.heartbeatAgeSec < 300;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="space-y-3 sm:space-y-0 sm:flex sm:flex-row sm:flex-wrap sm:items-start sm:justify-between sm:gap-4">
        ...
        <div className="flex items-start min-w-0 gap-3 sm:flex-1">
      ...
      {showHelp && (
        <div className="rounded-2xl border border-[#E4E4E7] bg-[#FAFAFA] p-5 text-[13.5px] text-[#3F3F46]">
          ...
          <ol className="list-decimal ml-5 space-y-1.5">
            ...
            <li>
              {t("vfStep3prefix")}
              <b>{t("bibiVesselSync")}</b>
              {t("vfStep3middle")}
              <code className="bg-white border border-[#E4E4E7] rounded px-1.5 py-0.5 text-[12px]">
                {t("vfStep3yourSiteUrl")}
              </code>
              {t("vfStep3suffix")}
            </li>
            <li>
              ...
              <a
                ...
                rel="noreferrer"
              >
                {t("adm_vesselfindercom")}
              </a>
              {t("vfStep4suffix")}
            </li>
            <li>
              {t("vfStep5prefix")}
              <b>{t("vfStep5connect")}</b>
              {t("vfStep5suffix")}
              <b>{t("vfStep5online")}</b>
              {t("vfStep5dot")}
            </li>
          ...
          <div className="mt-4 pt-3 border-t border-[#E4E4E7] text-[12.5px] text-[#52525B]">
            <b className="text-[#18181B]">
              BIBI Cars (auction parser) extension — required keys:
            </b>
            <ol className="list-decimal ml-5 mt-1.5 space-y-1">
              <li>
                Scroll down to the <b>“Extension keys”</b> block on this page
                and click <b>“Generate new client”</b>.
              </li>
              <li>
                Copy{" "}
                <code className="bg-white border border-[#E4E4E7] rounded px-1 text-[11px]">
                  Client ID
                </code>{" "}
                and{" "}
                <code className="bg-white border border-[#E4E4E7] rounded px-1 text-[11px]">
              ...
              <li>
                Open the BIBI Cars extension popup → paste your CRM URL → paste
                the keys into <b>Client ID</b> / <b>Client Secret</b> fields →{" "}
                <b>Save</b>.
              </li>
              <li>
                The “missing keys” warning will disappear and the extension
                starts sending HMAC-signed observations.
              </li>
            </ol>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-[#E4E4E7] bg-white p-4 sm:p-5 md:p-6">
        <div className="flex flex-col gap-3 mb-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between sm:gap-4 sm:mb-5">
          <div className="flex-1 min-w-0">
            <h3 className="text-[15px] sm:text-base md:text-lg font-semibold text-[#18181B]">
              Extension keys
            </h3>
            <p className="mt-1 text-[12px] sm:text-sm text-[#71717A] max-w-2xl leading-relaxed">
              Vessel Sync uses the{" "}
              <b className="text-[#18181B]">Shared HMAC secret</b> below. The
              BIBI Cars (auction parser) extension uses
              <b className="text-[#18181B]"> per-machine</b> client IDs/secrets
              — generate them in the second block.
            </p>
          </div>
          <button
        ...
        <div className="mb-4 rounded-2xl border border-[#E4E4E7] bg-white p-4">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11.5px] text-[#71717A] mt-1.5 leading-relaxed break-words">
                {sharedSecret?.configured ? (
                  <>
                    Source:{" "}
                    <code className="bg-zinc-50 border border-[#E4E4E7] rounded px-1.5 py-0.5 text-[11px] text-[#18181B]">
                      {sharedSecret.source}
                    </code>{" "}
                    · Fingerprint:{" "}
                    <code className="bg-zinc-50 border border-[#E4E4E7] rounded px-1.5 py-0.5 text-[11px] text-[#18181B]">
                      {sharedSecret.fingerprint}…
                    </code>{" "}
            ...
            {sharedSecret?.configured && (
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  type="button"
                  onClick={() =>
                    copyToClipboard(sharedSecret.secret, "shared-secret")
                  }
          ...
          {sharedSecret?.configured && (
            ...
            >
              {sharedSecretVisible
                ? sharedSecret.secret
                : "•".repeat(Math.min(48, sharedSecret.length || 48))}
            </code>
          )}
          {sharedSecret?.usage && (
            <div className="mt-2 text-[11.5px] text-[#71717A] leading-relaxed">
              {sharedSecret.usage}
            </div>
          )}
        </div>
        ...
        {newSecret && (
          <div className="mb-3 rounded-2xl border border-[#E4E4E7] bg-white p-4">
            <div className="flex items-center justify-between gap-3 mb-2">
              <div className="flex items-center gap-2 text-[13px] font-semibold text-[#18181B] min-w-0">
                ...
                <span className="truncate">
                  New keys for: {newSecret.name || newSecret.clientId}
                </span>
              </div>
              <button
            ...
            <div className="text-[11.5px] text-[#71717A] mb-3 leading-relaxed">
              <b className="text-[#18181B]">Copy the secret now.</b> It is
              hashed on the server and cannot be shown again. Rotate to issue a
              new one.
            </div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <div className="text-[10.5px] uppercase tracking-wider text-[#71717A] font-semibold mb-1.5">
                  Client ID
                </div>
                <div className="flex items-center gap-2 bg-zinc-50 rounded-xl border border-[#E4E4E7] px-3 py-2">
                  <code
                    className="text-[12px] text-[#18181B] flex-1 break-all"
                    data-testid="vf-ext-new-client-id"
                  >
                    {newSecret.clientId}
                  </code>
                  <button
              ...
              <div>
                ...
                <div className="flex items-center gap-2 bg-zinc-50 rounded-xl border border-[#E4E4E7] px-3 py-2">
                  <code
                    className="text-[12px] text-[#18181B] flex-1 break-all"
                    data-testid="vf-ext-new-client-secret"
                  >
                    {newSecret.secret}
                  </code>
                  <button
                    ...
                    onClick={() =>
                      copyToClipboard(
                        newSecret.secret,
                        `new-${newSecret.clientId}-secret`,
                      )
                    }
                    className="text-[11px] text-[#18181B] underline hover:no-underline whitespace-nowrap"
                  >
                    {copiedField === `new-${newSecret.clientId}-secret`
                      ? "copied"
                      : "copy"}
                  </button>
                </div>
        ...
        {extClients.length === 0 ? (
          <div className="text-[12.5px] text-[#71717A] py-2">
            No per-machine extension clients yet. Click{" "}
            <b>Generate new client</b> above to create one (only needed for the{" "}
            <b>BIBI Cars (auction parser)</b> extension — Vessel Sync uses the
            shared secret above).
          </div>
        ) : (
              ...
              <tbody>
                {extClients.map((c) => {
                  ...
                  return (
                    <tr
                      key={c.clientId}
                      className="border-b border-[#F4F4F5] align-top"
                    >
                      <td className="py-2 pr-3">
                        <div className="flex items-center gap-2">
                          <code className="text-[12px] text-[#18181B] break-all">
                            {c.clientId}
                          </code>
                          <button
                            type="button"
                          ...
                          >
                            {copiedField === `list-${c.clientId}-id`
                              ? "copied"
                              : "copy"}
                          </button>
                        </div>
                      </td>
                      <td className="py-2 pr-3 text-[#18181B]">
                        {c.name || "—"}
                      </td>
                      <td className="py-2 pr-3">
                        {cached ? (
                            ...
                            <div className="flex flex-col gap-0.5">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (
                                    window.confirm(
                                      "Forget this secret from local cache?",
                                    )
                                  )
                                    forgetCachedSecret(c.clientId);
                                }}
                          ...
                          <span className="text-[11px] text-[#A1A1AA] italic">
                            not cached — rotate to issue a new one
                          </span>
                        )}
                      </td>
                      <td className="py-2 pr-3">
      ...
      <section className="rounded-2xl border border-[#E4E4E7] bg-white p-4 sm:p-5">
        <div className="mb-4 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill kind={extensionOk ? "healthy" : "expired"}>
              {t("vfPillStep1Prefix")}{" "}
              {extensionOk ? t("vfPillStep1Working") : t("vfPillStep1Offline")}
            </StatusPill>
            <StatusPill
              kind={
                vfFetchOkOrMatch
            ...
            <StatusPill
              kind={
                matchOk ? "healthy" : vfFetchOkOrMatch ? "degraded" : "offline"
              }
            >
              {t("vfPillStep3Prefix")} {matchOk ? "✓" : "—"}
            </StatusPill>
            {status?.extensionVersion && (
              <span className="text-[11.5px] text-[#71717A] ml-1">
                ext v{status.extensionVersion}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 px-1 -mx-1 overflow-x-auto flex-nowrap sm:flex-wrap sm:overflow-visible sm:mx-0 sm:px-0 no-scrollbar">
            <button
              onClick={pingSession}
            ...
            >
              ...
              <span>{t("vfBtnResetCounters")}</span>
            </button>
            <button
            ...
            >
              <Target size={14} weight="duotone" />{" "}
              <span className="whitespace-nowrap">{t("vfBtnCreateTest")}</span>
            </button>
            <button
            ...
            >
              <ArrowClockwise size={14} />{" "}
              <span className="whitespace-nowrap">{t("vfBtnRemoveTest")}</span>
            </button>
            <button
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-5 sm:gap-3">
          <Stat
            label={t("vfStatActiveShipments")}
            value={status?.activeShipments ?? 0}
            icon={CheckCircle}
            sub={
              status?.activeShipments
                ? t("vfStatTracking")
                : t("vfStatNothingTracked")
            }
            tone={status?.activeShipments ? "emerald" : "amber"}
          />
          <Stat
            label={t("heartbeatLabel")}
            value={
              status?.heartbeatAgeSec != null
                ? fmtAgo(status.lastHeartbeatAt)
                : "—"
            }
            sub={extensionOk ? t("vfStatExtAlive") : t("vfStatExtNoSignal")}
            tone={extensionOk ? "emerald" : "rose"}
          />
          <Stat
            label={t("vfStatVfResponds")}
            value={
              status?.vfFetchOkCount != null
                ? status.vfFetchOkCount + (status?.successCount || 0)
                : "—"
            }
            sub={
              status?.lastVfFetchOkAt
                ? fmtAgo(status.lastVfFetchOkAt)
                : status?.lastSuccessAt
                  ? fmtAgo(status.lastSuccessAt)
                  : t("vfStatVfNoSuccess")
            }
            tone={vfFetchOkOrMatch ? "emerald" : "slate"}
          />
          <Stat
            label={t("vfStatOurMatches")}
            value={status?.successCount ?? 0}
            sub={
              status?.lastSuccessAt
                ? fmtAgo(status.lastSuccessAt)
                : t("vfStatNoMatchesYet")
            }
            tone={matchOk ? "emerald" : "slate"}
          />
          <Stat
            label={t("vfStatLastReason")}
            value={
              status?.consecutiveFails != null
                ? `${status.consecutiveFails} ${t("vfStatConsecutive")}`
                : "—"
            }
            sub={status?.lastFailReason || t("vfStatOkLabel")}
            tone={status?.consecutiveFails > 5 ? "amber" : "slate"}
          />
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-[#E4E4E7] bg-white p-4 sm:p-5 md:p-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <input
            ...
            placeholder={t(
              "adm_msc_oscar_wbaja7c52kww12345_227280290_mscu1234567",
            )}
        ...
        {searchData && (
          <div className="mt-4 space-y-3">
            <div className="text-xs text-[#71717A] flex items-center gap-3">
              <span>
                {t("adm_found")} <b className="text-[#18181B]">{totalFound}</b>
              </span>
              {classification && (
            ...
            {liveVessels.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-[#18181B] mb-2 flex items-center gap-1.5">
                  <Boat size={14} weight="fill" className="text-[#71717A]" />{" "}
                  {t("r9_live_vessels")} (VesselFinder) — {liveVessels.length}
                </div>
                <div className="overflow-x-auto rounded-xl border border-[#E4E4E7]">
                  <table className="w-full text-xs">
                    <thead className="bg-zinc-50 text-[#71717A]">
                      <tr>
                        <th className="p-2.5 text-left font-semibold uppercase tracking-wider text-[10.5px]">
                          {t("name")}
                        </th>
                        <th className="p-2.5 text-left font-semibold uppercase tracking-wider text-[10.5px]">
                          MMSI
                    ...
                    <tbody>
                      {liveVessels.map((v, i) => (
                        ...
                        >
                          ...
                          <td className="p-2.5 text-[#18181B]">
                            {v.speed ?? "—"} kn
                          </td>
                          <td className="p-2.5">
                            <button
            ...
            {richShipments.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-[#18181B] mb-2 flex items-center gap-1.5">
                  <Target size={14} weight="fill" className="text-[#71717A]" />{" "}
                  Shipments (VIN / container / vessel) — {richShipments.length}
                </div>
                    ...
                    <tbody>
                      {richShipments.map((s) => {
                        const healthDot =
                          s.trackingHealth === "ok"
                            ? "#16A34A"
                            : s.trackingHealth === "stale"
                              ? "#DC2626"
                              : s.trackingHealth === "estimated"
                                ? "#F59E0B"
                                : "#A1A1AA";
                        const healthLabel =
                          s.trackingHealth === "ok"
                        ...
                        return (
                          <tr
                            key={s.id}
                            className="border-t border-[#F4F4F5] hover:bg-zinc-50/60"
                            data-testid={`rich-shipment-${s.id}`}
                          >
                            <td className="p-2.5">
                              <div className="font-mono text-[11px] text-[#18181B]">
                                {s.vin || "—"}
                              </div>
                            ...
                            <td className="p-2.5 w-28">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[10px] font-semibold text-[#18181B] w-7 text-right">
            ...
            {dbShipments.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-[#18181B] mb-2 flex items-center gap-1.5">
                  <Target size={14} weight="fill" className="text-[#71717A]" />{" "}
                  {t("r9_shipments_in_db")} — {dbShipments.length}
                </div>
                <div className="space-y-1.5">
                  {dbShipments.map((s) => (
                    <div
                      key={s.id}
                      className="rounded-xl border border-[#E4E4E7] p-3 text-xs flex items-center gap-3 hover:bg-zinc-50/60"
                    >
                      <div className="flex-1 min-w-0">
            ...
            {totalFound === 0 && !searching && (
              <div className="rounded-xl border border-dashed border-[#E4E4E7] p-6 text-center text-xs text-[#71717A]">
                {t("r9_nothing_found")}.{" "}
                {searchData?.live?.error ? (
                  <span className="text-[#DC2626]">
                    Live: {String(searchData.live.error).slice(0, 120)}
                  </span>
                ) : null}
              </div>
            )}
      ...
      <section className="rounded-2xl border border-[#E4E4E7] bg-white p-4 sm:p-5 md:p-6">
        <div className="flex items-center justify-between gap-2 mb-4">
          ...
          <button
            ...
            aria-label={t("vfBtnRefreshTitle")}
            title={t("vfBtnRefreshTitle")}
          >
        ...
        {shipments.length === 0 ? (
          <div className="text-center py-10 text-sm text-[#71717A]">
            {t("vfShipmentsEmpty")}
          </div>
        ) : (
          <div className="overflow-x-auto rounded-xl border border-[#E4E4E7]">
                ...
                {shipments.map((s) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2.5">
                        {s.vessel?.name ? (
                          <>
                            ...
                            <div className="font-mono text-[10px] text-[#71717A]">
                              {s.vessel.mmsi ? `MMSI:${s.vessel.mmsi}` : ""}{" "}
                              {s.vessel.imo ? `IMO:${s.vessel.imo}` : ""}
                            </div>
                          </>
                        ) : (
                          <span className="text-[#A1A1AA]">
                            {t("adm_not_assigned_2")}
                          </span>
                        )}
                      </td>
                      <td className="p-2.5 font-mono text-[10px] text-[#18181B]">
                      ...
                      <td className="p-2.5">
                        {s.trackingActive ? (
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#E4E4E7] bg-white px-2 py-0.5 text-[10px] font-semibold text-[#18181B]">
                            <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#16A34A]" />{" "}
                            ON
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#E4E4E7] bg-white px-2 py-0.5 text-[10px] font-semibold text-[#71717A]">
                            <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#A1A1AA]" />{" "}
                            OFF
                          </span>
                        )}
                      </td>
                      <td className="p-2.5 text-[10px]">
                        {r ? (
                          r.ok ? (
                            ...
                            <span className="text-[#DC2626]" title={r.error}>
      ...
      <section
        id="bind-card"
        className="rounded-2xl border border-[#E4E4E7] bg-white p-4 sm:p-5 md:p-6"
      >
        ...
        <p className="text-xs text-[#71717A] mb-4 leading-relaxed">
          {t("vfBindIntro1")}
          <b>VIN</b>
          {t("vfBindIntro2")} {t("vfBindIntro3")}
          <b>{t("vfBindIntroBold")}</b>
          {t("vfBindIntro4")}
        </p>
        ...
        <div className="flex flex-wrap items-center gap-3 mt-4">
          <button
            ...
            disabled={
              bindBusy ||
              (!bindShipmentId && !bindVin.trim()) ||
              (!bindMmsi && !bindImo && !bindName)
            }
          ...
          >
            <Warning size={14} weight="fill" className="text-[#F59E0B]" />{" "}
        ...
        {bindResult?.ok && (
          <div className="mt-3 rounded-xl border border-[#E4E4E7] bg-white px-3 py-2.5 text-sm text-[#18181B] flex items-start gap-2">
            <CheckCircle
              size={16}
              weight="fill"
              className={`mt-0.5 flex-shrink-0 ${bindResult.data?.createdNewStage ? "text-[#F59E0B]" : "text-[#16A34A]"}`}
            />
            <div className="flex-1">
              ...
              <div className="text-xs mt-0.5 text-[#71717A]">
                {t("shipmentAlerts")}
                <span className="font-mono text-[#18181B]">
                  {bindResult.data?.shipmentId}
                </span>
                {t("vfBindStageCount")}
                <b className="text-[#18181B]">
                  {bindResult.data?.vesselStagesCount}
                </b>
                {bindResult.data?.container && (
                  <>
                    {" "}
                    {t("adm_container_2")}{" "}
                    <span className="font-mono text-[#18181B]">
                      {bindResult.data.container.number}
                    </span>
                  </>
                )}
              </div>
                ...
                return (
                  <div key={st.stageId} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-8 h-8 rounded-full ${dot} flex items-center justify-center`}
                      >
                        {isDone ? (
                          <CheckCircle
                            size={14}
                      ...
                      {i < vesselHistory.vesselStages.length - 1 && (
                        <div
                          className={`flex-1 w-0.5 my-1 ${line}`}
                    ...
                    <div className="flex-1 pb-5">
                      <div className="flex flex-wrap items-baseline gap-2">
                        ...
                        {isCurrent && (
                          <span className="text-[10px] uppercase tracking-wider text-[#18181B] font-bold">
                            {t("vfStageActive")}
                          </span>
                        )}
                        {isDone && (
                          <span className="text-[10px] uppercase tracking-wider text-[#16A34A] font-semibold">
                            {t("vfStageDone")}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="text-[11px] flex flex-wrap gap-1.5 mt-2">
                        {st.vessel?.name && (
                          <span className="inline-flex items-center gap-1 font-mono bg-zinc-50 text-[#18181B] border border-[#E4E4E7] px-2 py-0.5 rounded-lg">
                            <Anchor size={10} className="text-[#71717A]" />{" "}
                            {st.vessel.name}
                          </span>
                        )}
                        {st.vessel?.mmsi && (
                      ...
                      {(st.startedAt || st.completedAt) && (
                        <div className="text-[10px] text-[#A1A1AA] mt-1.5 font-mono">
                          {st.startedAt && (
                            <span>
                              {t("adm3_4454f5463a")} {fmtAgo(st.startedAt)}
                            </span>
                          )}
                          {st.completedAt && (
                            <span className="ml-3">
                              {t("adm3_a63ec7aa83")} {fmtAgo(st.completedAt)}
                            </span>
                          )}
                        </div>
                      )}
