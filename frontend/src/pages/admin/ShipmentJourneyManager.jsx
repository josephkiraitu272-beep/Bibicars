...
export default function ShipmentJourneyManager() {
  ...
  const loadJourney = useCallback(async (id) => {
    ...
    try {
      const { data } = await axios.get(
        `${API_URL}/api/shipments/${id}/journey`,
      );
  ...
  useEffect(() => {
    if (selectedId) loadJourney(selectedId);
  }, [selectedId, loadJourney, refreshKey]);
  ...
  const filtered = list.filter((s) => {
    ...
    return (
      ...
      String(s.vin || "")
        .toLowerCase()
        .includes(q) ||
      String(s.vehicleTitle || "")
        .toLowerCase()
        .includes(q) ||
      String(s.containerNumber || "")
        .toLowerCase()
        .includes(q)
    );
  });
  ...
  const onAdvance = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${API_URL}/api/shipments/${journey.id}/stages/advance`,
      );
      if (data?.ok) {
        toast.success(t("adm_go_to_the_next_stage"));
        bump();
      }
    } catch (e) {
  ...
  const onActivate = async (stageId) => {
    ...
    try {
      const { data } = await axios.post(
        `${API_URL}/api/shipments/${journey.id}/stages/${stageId}/activate`,
      );
      if (data?.ok) {
        toast.success(t("adm_stage_activated"));
        bump();
      }
    } catch (e) {
  ...
  const onBindVessel = async (stageId, form) => {
    ...
    try {
      const { data } = await axios.put(
        `${API_URL}/api/shipments/${journey.id}/stages/${stageId}`,
        {
          vessel: {
            name: form.name || null,
            mmsi: form.mmsi || null,
      ...
      if (data?.ok) {
        toast.success(t("adm_vessel_linked"));
        bump();
      }
    } catch (e) {
    ...
    <div className="space-y-6" data-testid="shipment-journey-manager">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-zinc-900">
          {t("adm_delivery_route_management")}
        </h1>
        <p className="text-zinc-500 text-sm mt-1">
          {t("adm_stages_vessels_position_and_manual_switches")}
        </p>
      </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          {selectedId && journey && (
            <>
              {/* Controls bar */}
              <div
                className="flex flex-wrap items-center gap-2 bg-white rounded-2xl border border-zinc-200 p-3"
                data-testid="sjm-controls"
              >
                <button
                  onClick={onAdvance}
                ...
                {currentStage && (
                  <div className="ml-auto text-sm text-zinc-600">
                    ...
                    <span className="font-medium">
                      {STAGE_TYPE_LABEL[currentStage.type] || currentStage.type}
                    </span>
...
function VesselBindCard({ stage, onSubmit }) {
  ...
  return (
    <div
      ...
      data-testid="sjm-vessel-bind"
    >
      <div className="flex items-center gap-2 mb-3">
        <Anchor size={18} className="text-blue-600" />
        <h3 className="font-semibold text-zinc-900">
          {t("adm_bind_vessel_to_current_stage")}
        </h3>
      </div>
