...
const BUCKETS = {
  hot: {
    labelKey: "hotLeadsCount",
    icon: Fire,
    color: "red",
    bg: "bg-red-500",
  },
  warm: {
    labelKey: "warmLeadsCount",
    icon: ThermometerSimple,
    color: "amber",
    bg: "bg-amber-500",
  },
  cold: {
    labelKey: "coldLeadsCount",
    icon: Snowflake,
    color: "blue",
    bg: "bg-blue-500",
  },
...
const LeadCard = ({ lead, onAction }) => {
  ...
  return (
    <div
      className="bg-white rounded-xl border border-zinc-200 p-4 hover:shadow-lg transition-all"
    ...
    >
      ...
      <div className="space-y-2 mb-4">
        <ScoreBar
          label={t("behavior")}
          value={lead.behaviorScore || 0}
          color="violet"
        />
        <ScoreBar
          label={t("sales")}
          value={lead.salesScore || 0}
          color="blue"
        />
        <ScoreBar
          label={t("deal")}
          value={lead.dealScore || 0}
          color="emerald"
        />
      </div>
      {lead.nextAction && (
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-zinc-100">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-medium"
          data-testid={`call-lead-${lead.id}`}
        >
          <Phone size={16} /> {t("call")}
        </button>
        <button
          onClick={() => onAction(lead.id, "view")}
          className="p-2 rounded-lg border border-zinc-200 text-zinc-600 hover:bg-zinc-50"
  ...
  return (
    <div className="space-y-6" data-testid="predictive-leads-page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          icon={Fire}
          title={t("hotLeadsCount")}
          value={hotLeads.length}
          color="red"
        />
        <StatCard
          icon={ThermometerSimple}
          title={t("warmLeadsCount")}
          value={warmLeads.length}
          color="amber"
        />
        <StatCard
          icon={Snowflake}
          title={t("coldLeadsCount")}
        ...
        <StatCard
          icon={Target}
          title={t("total")}
          value={hotLeads.length + warmLeads.length + coldLeads.length}
          color="violet"
        />
      </div>
        ...
        {Object.entries(BUCKETS).map(([key, config]) => {
          ...
          return (
            <div key={key} className="space-y-4">
              <div
                className={`flex items-center justify-between p-4 rounded-xl bg-${config.color}-100`}
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span
                    className={`text-lg font-bold text-${config.color}-700`}
                  >
                    {t(config.labelKey)}
                  </span>
                </div>
                <span
                  className={`px-3 py-1 rounded-full bg-white text-${config.color}-600 font-bold`}
                >
                  {leads.length}
                </span>
              </div>
              <div className="space-y-4">
                {leads.length === 0 ? (
                  <div className="p-6 rounded-xl border-2 border-dashed border-zinc-200 text-center text-zinc-400">
                    {t("noLeadsInBucket")}
                  </div>
                ) : (
                  leads.map((lead) => (
