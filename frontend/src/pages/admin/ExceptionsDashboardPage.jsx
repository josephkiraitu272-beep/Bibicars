...
function fmtAgo(iso, t) {
  ...
  if (s < 60)
    return `${Math.round(s)} ${tr("r9_seconds_ago") || "seconds ago"}`;
  if (s < 3600)
    return `${Math.round(s / 60)} ${tr("r9_minutes_ago") || "minutes ago"}`;
  if (s < 86400)
    return `${Math.round(s / 3600)} ${tr("r9_hours_ago") || "hours ago"}`;
...
export default function ExceptionsDashboardPage() {
  ...
  const runResolverFor = useCallback(
    async (shipmentId) => {
      setResolverBusyId(shipmentId);
      setResolverMsg(null);
      try {
        const r = await axios.post(
          `${API_URL}/api/admin/shipments/${shipmentId}/resolver/run`,
        );
        ...
        if (!parts.length) {
          ...
          parts.push(
            `${t("r9_nothing_new")}container ${confC}%, vessel ${confV}%)`,
          );
        }
        setResolverMsg({
          kind: "ok",
          text: `${shipmentId}: ${parts.join(" · ")}`,
        });
        await reload();
      } catch (e) {
        setResolverMsg({
          kind: "err",
          text: `${shipmentId}: ${e?.response?.data?.detail || String(e)}`,
        });
      } finally {
  ...
  const runResolverQueue = useCallback(async () => {
    ...
    try {
      const r = await axios.post(
        `${API_URL}/api/admin/resolver/run-queue?limit=20`,
      );
      const d = r.data || {};
      setResolverMsg({
        kind: "ok",
  ...
  const itemsToShow =
    activeBucket === "all" ? allItems : buckets[activeBucket] || [];
    ...
    <div className="space-y-4 sm:space-y-5" data-testid="exceptions-dashboard">
      ...
      */}
      ...
      {resolverQueue && resolverQueue.total > 0 && (
        <div
          className="p-5 border-2 border-indigo-200 shadow-sm rounded-xl bg-gradient-to-br from-indigo-50 via-white to-white"
          data-testid="resolver-queue"
        >
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-indigo-800 mt-0.5">
                  {t("adm_shipments_without")} <b>container</b> {t("adm_or")}{" "}
                  <b>vessel</b> {t("adm3_cc8fbb20a2")}
                </p>
              </div>
            </div>
            <button
            ...
            >
              <Lightning
                size={14}
                weight="fill"
                className={resolverBusy ? "animate-pulse" : ""}
              />
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            {resolverQueue.buckets?.missing_container > 0 && (
              <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-2.5 py-1 text-[11px] text-amber-800">
                {t("adm_no_container_2")}{" "}
                <b>{resolverQueue.buckets.missing_container}</b>
              </span>
            )}
            {resolverQueue.buckets?.missing_vessel > 0 && (
              <span className="inline-flex items-center gap-1 rounded-full bg-sky-50 border border-sky-200 px-2.5 py-1 text-[11px] text-sky-800">
                {t("adm_no_vessel_2")}{" "}
                <b>{resolverQueue.buckets.missing_vessel}</b>
              </span>
            )}
          ...
          {resolverMsg && (
            <div
              className={`rounded-md border px-3 py-1.5 mb-3 text-xs ${
                resolverMsg.kind === "ok"
          ...
          {/* rows */}
          <div className="overflow-hidden bg-white border border-indigo-100 rounded-lg">
            <table className="w-full text-sm">
              <thead className="border-b border-indigo-100 bg-indigo-50/60">
                <tr>
                  <th className="px-3 py-2 text-left text-[11px] font-semibold text-indigo-700 uppercase tracking-wide">
                    {t("adm_vin_shipment")}
                  </th>
                  <th className="px-3 py-2 text-left text-[11px] font-semibold text-indigo-700 uppercase tracking-wide">
                    {t("missingLabel")}
                ...
                {(resolverQueue.items || []).map((it) => {
                  ...
                  return (
                    <tr
                      ...
                      data-testid={`resolver-row-${it.id}`}
                    >
                      ...
                      <td className="px-3 py-2.5 align-top">
                        ...
                        {it.containerConfidence != null && (
                          <div className="text-[10px] text-slate-500 mt-0.5">
                            conf {Math.round(it.containerConfidence * 100)}%
                          </div>
                        )}
                      </td>
                      <td className="px-3 py-2.5 align-top">
                        ...
                        {r.lastRun ? (
                          <>
                            <div className="font-medium text-slate-700">
                              {new Date(r.lastRun).toLocaleString(getLocale(), {
                                day: "2-digit",
                                month: "short",
                                hour: "2-digit",
                                minute: "2-digit",
      ...
      {!loading && itemsToShow.length === 0 && (
        <div
          className="p-8 text-center border rounded-xl border-emerald-200 bg-emerald-50"
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, Math.max(0, Math.round((it.progress || 0) * 100)))}%`,
                          }}
                        />
                      </div>
                      <span className="w-8 text-xs font-semibold text-right text-slate-700">
                  ...
                  <td className="px-4 py-3">
                    <div className="text-xs text-slate-700">
                      {fmtAgo(it.lastTrackingUpdate, t)}
                    </div>
                    {it.ageHours !== null && it.ageHours !== undefined && (
                      <div className="text-[10px] text-slate-400">
                        {it.ageHours} {t("adm3_dd70ac60ce")}
                      </div>
                    )}
                  </td>
  ...
  return (
    ...
    >
      <Icon
        ...
        style={!active ? { color: dot } : undefined}
      />
      <span className="font-semibold text-[12.5px] truncate">{label}</span>
      <span
        className={`ml-auto inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-[10.5px] font-semibold ${
          active ? "bg-white/15 text-white" : "bg-[#F4F4F5] text-[#71717A]"
        }`}
      >
        {count}
      </span>
