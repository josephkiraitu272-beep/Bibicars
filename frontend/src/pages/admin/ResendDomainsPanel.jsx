...
export default function ResendDomainsPanel({ hasApiKey }) {
  ...
  const fetchDetail = useCallback(async (domainId) => {
    try {
      const r = await axios.get(
        `${API_URL}/api/admin/integrations/resend/domains/${domainId}`,
        { headers: authHeaders() },
      );
  ...
  const handleVerify = async (domainId) => {
    ...
    try {
      ...
      if (fresh?.status === "verified") {
        ...
        toast.info(
          t("resendDomainStatusUnknown").replace(
            "{status}",
  ...
  const handleDelete = async (domain) => {
    if (
      !window.confirm(
        t("resendConfirmDeleteDomain").replace("{name}", domain.name),
      )
    )
      return;
    ...
    <div className="space-y-3">
      ...
      {showAdd && (
        <div className="rounded-xl border border-[#E4E4E7] bg-[#FAFAFA] p-3 space-y-2.5">
          ...
          <p className="text-[11px] text-[#71717A] leading-relaxed">
            ...
            дождитесь распространения (5-30 мин), затем нажмите{" "}
            <b>Verify now</b>.
          </p>
        </div>
      )}
      ...
      {items.length === 0 && !loading ? (
        <div className="rounded-xl border border-dashed border-[#E4E4E7] p-6 text-center text-[12.5px] text-[#71717A]">
          В вашем Resend-аккаунте ещё нет доменов. Нажмите <b>Add domain</b>{" "}
              ...
              >
                ...
                <div className="px-3.5 py-2.5 flex items-center gap-3 flex-wrap">
                  ...
                  >
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10.5px] font-semibold ring-1 ${s.color} shrink-0`}
                    >
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {!isVerified && (
                      ...
                      >
                        <RefreshCw
                          className={`w-3 h-3 ${verifying[d.id] ? "animate-spin" : ""}`}
                        />
                    ...
                    {!detail ? (
                      ...
                      <>
                                ...
                                return (
                                  ...
                                  >
                                    <td className="py-2 pr-2">
                                      <span
                                        className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold ring-1 ${rs.color}`}
                                      >
                        ...
                        <div className="rounded-lg bg-white border border-[#E4E4E7] px-3 py-2 text-[11px] text-[#71717A] leading-relaxed">
                          ...
                          {t("resendDnsInstr")} <b>Verify now</b>.
                        </div>
                      </>
                    )}
                  </div>
