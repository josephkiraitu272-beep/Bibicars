...
const QUICK_FILTERS = [
  ...
  },
  {
    ...
    icon: CalendarBlank,
    color: "#71717A",
    labelKey: "tasksFilterNoDeadline",
...
const ORPHAN_PANELS = [
  {
    id: "leads",
    icon: CalendarSlash,
    labelKey: "tasksFilterLeadsWoTasks",
    fallback: "Leads w/o tasks",
    endpoint: "/api/tasks/reports/leads-without-tasks",
    linkPrefix: "/admin/leads",
  },
  {
    id: "customers",
    icon: CalendarDots,
    labelKey: "tasksFilterCustomersWoTasks",
    fallback: "Customers w/o tasks",
    endpoint: "/api/tasks/reports/customers-without-tasks",
...
const Tasks = () => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      else if (quickFilter === "no_deadline")
        params.append("filter", "no_deadline");
      ...
      if (quickFilter === "open") {
        ...
        try {
          const r2 = await axios.get(`${API_URL}/api/tasks?${p2}`, {
            headers: authHeaders(),
          });
  ...
  const fetchOrphans = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}${panel.endpoint}?${params}`, {
        headers: authHeaders(),
      });
  ...
  const loadAssignees = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/tasks/eligible-assignees`, {
        headers: authHeaders(),
      });
  ...
  const openCreateModal = async () => {
    if (!canCreateTasks) {
      toast.error(
        tt(t, "tasksRoleNotAllowed", "Your role cannot create tasks."),
      );
      return;
    }
    setShowModal(true);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title.trim()) {
      toast.error((t("taskTitle") || "Title") + " — required");
      return;
    }
    if (!formData.assigneeId) {
      toast.error(tt(t, "pickAssignee", "Please pick an assignee"));
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.error(
        err?.response?.data?.detail || t("error") || "Failed to create task",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    return (
      ...
      >
        <Icon
          size={14}
          weight="bold"
          style={{ color: active ? "#fff" : filter.color }}
        />
    ...
    >
      ...
      <div className="flex flex-row items-start justify-between gap-3 sm:gap-4 mb-6 lg:mb-8">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <RefreshButton
            onClick={orphanMode ? fetchOrphans : fetchTasks}
            loading={loading}
            ariaLabel={t("adm_refresh_3") || "Refresh"}
            testId="tasks-refresh-btn"
          />
          {canCreateTasks && (
            <button
      ...
      <div className="card p-4 sm:p-5 mb-5 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {QUICK_FILTERS.map((f) => (
            <Pill key={f.id} filter={f} />
          ))}
        </div>

        <div className="flex flex-wrap gap-2">
          {ORPHAN_PANELS.map((p) => (
            <OrphanPill key={p.id} panel={p} />
          ))}
        </div>
        ...
        {canSeeAssigneeFilter && (
          <div className="flex items-center gap-2 pt-2 border-t border-[#F4F4F5]">
            ...
            >
              <SelectTrigger
                className="w-full sm:w-[280px] input"
                data-testid="tasks-assignee-filter"
              >
                <SelectValue
                  placeholder={tt(t, "allManagers", "All managers")}
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">
                  {tt(t, "allManagers", "All managers")}
                </SelectItem>
                {assigneeOptions.map((opt) => (
                  <SelectItem key={opt.id} value={opt.id}>
                    {opt.displayName || opt.name || opt.email} ·{" "}
                    {ROLE_BADGE[opt.role]?.label || opt.role}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-[#71717A]">
              {tt(
                t,
                "tasksOrphanEmpty",
                "Everyone has at least one open task — nothing to act on.",
              )}
            </div>
          ) : (
            orphanItems.map((it) => {
              ...
              return (
                <div
                  ...
                  data-testid={`tasks-orphan-row-${it.id}`}
                >
                  ...
                  <Link
                    to={link}
                    className="text-[#2563EB] hover:text-[#1D4ED8] inline-flex items-center gap-1 text-sm font-medium"
                  >
                    {tt(t, "tasksOpen", "Open")}
        ...
        <div className="space-y-3 sm:space-y-4">
          {loading ? (
            ...
            tasks.map((task) => {
              ...
              const assigneeBadge =
                ROLE_BADGE[(task.assigneeRole || "").toLowerCase()] || null;
              const customerLink = task.customerId
                ? `/admin/customers/${task.customerId}`
                : null;
              const leadLink = task.leadId
                ? `/admin/leads/${task.leadId}`
                : null;
              return (
                <div
                  key={task.id}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <h3 className="font-semibold text-[#18181B] break-words">
                          {task.title}
                        </h3>
                        <span
                          className="badge"
                        ...
                        {assigneeBadge && (
                          <span
                            className="badge"
                            style={{
                              backgroundColor: assigneeBadge.bg,
                              color: assigneeBadge.fg,
                            }}
                          >
                            <User size={11} weight="bold" />
                            <span className="ml-1">
                              {task.assigneeName || assigneeBadge.label}
                            </span>
                          </span>
                        )}
                      </div>
                      {task.description && (
                        <p className="text-sm text-[#71717A] mb-3 break-words">
                          {task.description}
                        </p>
                      )}
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-sm">
                        {task.dueDate && (
                          <div
                            className={`flex items-center gap-1.5 ${overdue ? "text-[#DC2626]" : "text-[#71717A]"}`}
                          >
                            {overdue ? (
                        ...
                        {!task.dueDate && (
                          <div className="flex items-center gap-1.5 text-[#A1A1AA]">
                            ...
                            <span>
                              {tt(t, "tasksNoDeadline", "No deadline")}
                            </span>
                          </div>
                        )}
                        {task.created_at && (
          ...
          >
            ...
            <div>
              ...
              >
                ...
                >
                  <SelectValue
                    placeholder={tt(t, "pickAssignee", "Pick an assignee")}
                  />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={tt(
                  t,
                  "tasksCommentPh",
                  "Anything the assignee should keep in mind…",
                )}
                className="input w-full resize-none"
  ...
  return (
    <div className="mt-3 flex items-start gap-2">
      <ChatCircleDots
        size={14}
        weight="bold"
        className="text-[#71717A] mt-2 shrink-0"
      />
