...
const Deals = () => {
  ...
  const statusLabels = {
    new: t("dealNew"),
    negotiation: t("dealNegotiation"),
    waiting_deposit: t("dealWaitingDeposit"),
    deposit_paid: t("dealDepositPaid"),
    purchased: t("dealPurchased"),
    in_delivery: t("dealInDelivery"),
    completed: t("dealCompleted"),
    cancelled: t("dealCancelled"),
  };
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await axios.patch(`${API_URL}/api/deals/${id}/status`, {
        status: newStatus,
      });
      ...
      toast.error(err.response?.data?.message || t("cannotChangeStatus"));
    }
  };

  const handleFinanceUpdate = async (id, field, value) => {
    try {
      await axios.patch(`${API_URL}/api/deals/${id}/finance`, {
        [field]: value,
      });
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1
              className="text-2xl font-bold tracking-tight text-[#18181B] leading-tight"
              style={{
                fontFamily:
      ...
      <div className="card p-4 sm:p-5 mb-4 sm:mb-5">
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <input
            ...
            className="input w-full sm:flex-1 sm:min-w-[200px]"
          ...
          >
            <SelectTrigger
              ...
              data-testid="deals-status-filter"
            >
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table
            className="table-premium min-w-[900px] w-full"
              ...
              {loading ? (
                ...
                deals.map((deal) => (
                  ...
                  >
                    ...
                    <td>
                      <div className="text-sm font-mono text-[#71717A]">
                        {deal.vin || "—"}
                      </div>
                      <div className="text-xs text-[#A1A1AA]">
                        {deal.vehicleTitle || deal.vehiclePlaceholder}
                      </div>
                    </td>
                    ...
                    <td onClick={(e) => e.stopPropagation()}>
                      ...
                      >
                        <SelectTrigger
                          className="w-[150px] h-8 bg-transparent border-0 p-0"
                          data-testid={`deal-status-${deal.id}`}
                        >
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-medium ${statusColors[deal.status]}`}
                          >
                    ...
                    <td
                      className={`font-medium ${(deal.estimatedMargin || 0) >= 0 ? "text-[#059669]" : "text-[#DC2626]"}`}
                    >
                      ${Math.abs(deal.estimatedMargin || 0).toLocaleString()}
                    </td>
                    <td
                      className={`font-semibold ${(deal.realProfit || 0) >= 0 ? "text-[#059669]" : "text-[#DC2626]"}`}
                    >
                      {deal.status === "completed"
                        ? `$${(deal.realProfit || 0).toLocaleString()}`
                        : "—"}
                    </td>
                    <td onClick={(e) => e.stopPropagation()}>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent
          className="max-w-[calc(100%-2rem)] sm:max-w-lg bg-white rounded-2xl border border-[#E4E4E7] max-h-[90vh] overflow-y-auto"
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      clientPrice: parseInt(e.target.value) || 0,
                    })
                  }
                  className="input w-full"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      internalCost: parseInt(e.target.value) || 0,
                    })
                  }
                  className="input w-full"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    vehiclePlaceholder: e.target.value,
                  })
                }
                placeholder={t("adm_bmw_x5_2022")}
                className="input w-full"
            ...
            <div className="bg-[#F4F4F5] rounded-xl p-4">
              ...
              <div
                className={`text-2xl font-bold ${formData.internalCost - formData.clientPrice >= 0 ? "text-[#059669]" : "text-[#DC2626]"}`}
              >
                $
                {Math.abs(
                  formData.internalCost - formData.clientPrice,
                ).toLocaleString()}
              </div>
            </div>
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent
          className="max-w-2xl bg-white rounded-2xl border border-[#E4E4E7]"
            ...
            <>
              ...
              <div className="space-y-6 mt-4">
                <div className="flex items-center justify-between">
                  <div>
                    <span
                      className={`px-3 py-1.5 rounded-full text-sm font-medium ${statusColors[selectedDeal.status]}`}
                    >
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <FinanceCard
                    ...
                    editable
                    onSave={(v) =>
                      handleFinanceUpdate(selectedDeal.id, "clientPrice", v)
                    }
                    t={t}
                  />
                  <FinanceCard
                    ...
                    editable
                    onSave={(v) =>
                      handleFinanceUpdate(selectedDeal.id, "internalCost", v)
                    }
                    t={t}
                  />
                  <FinanceCard
                    ...
                    editable
                    onSave={(v) =>
                      handleFinanceUpdate(selectedDeal.id, "purchasePrice", v)
                    }
                    t={t}
                  />
                  <FinanceCard
                    ...
                    editable
                    onSave={(v) =>
                      handleFinanceUpdate(selectedDeal.id, "realCost", v)
                    }
                    t={t}
                  />
                  <FinanceCard
                    ...
                    editable
                    onSave={(v) =>
                      handleFinanceUpdate(selectedDeal.id, "realRevenue", v)
                    }
                    t={t}
                  />
                </div>
                <div className="grid grid-cols-3 gap-4 bg-[#F9FAFB] rounded-xl p-4">
                  <div>
                    ...
                    <div
                      className={`text-xl font-bold ${(selectedDeal.estimatedMargin || 0) >= 0 ? "text-[#059669]" : "text-[#DC2626]"}`}
                    >
                      $
                      {Math.abs(
                        selectedDeal.estimatedMargin || 0,
                      ).toLocaleString()}
                    </div>
                  </div>
                  <div>
                    ...
                    <div
                      className={`text-xl font-bold ${(selectedDeal.realProfit || 0) >= 0 ? "text-[#059669]" : "text-[#DC2626]"}`}
                    >
                  ...
                  <div>
                    ...
                    <div
                      className={`text-xl font-bold ${(selectedDeal.overrideDelta || 0) > 0 ? "text-[#DC2626]" : "text-[#71717A]"}`}
                    >
                ...
                <DealPipelineAdvance
                  ...
                  onAdvanced={async () => {
                    const res = await axios.get(
                      `${API_URL}/api/deals/${selectedDeal.id}`,
                    );
                    setSelectedDeal(res.data);
                    fetchDeals();
                  }}
...
const DealPipelineAdvance = ({ deal, onAdvanced }) => {
  ...
  return (
    <div className="border-t border-[#E4E4E7] pt-4">
      <div className="text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
        {t("i18n_stage_d975f8")} «
        {(currentGroup?.labels && currentGroup.labels[lang]) ||
          currentGroup?.label ||
          "—"}
        »
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <span className="text-xs text-[#71717A]">
          {t("adm_current_stage_3")}
        </span>
        <span
          className="px-2.5 py-1 rounded-full text-xs font-semibold bg-[#4F46E5] text-white"
      ...
      {allowed.length === 0 ? (
        <p className="text-xs text-[#71717A]">
          {t("adm_final_stage_no_transitions")}
        </p>
      ) : (
