...
const NotificationsPage = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const notifRes = await fetch(`${API_URL}/api/notifications/me?limit=100`, {
      ...
      if (notifRes.ok) {
        ...
        const urgent = list.filter(n => (n.priority === 'urgent') || (n.severity === 'critical')).length;
        setStats({ total: list.length, unread, byPriority: { urgent }, today: 0 });
      }
  ...
  const markAsRead = async (id) => {
    try { await fetch(`${API_URL}/api/notifications/${id}/read`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` } }); setNotifications(prev => prev.map(n => n.id === id ? { ...n, isRead: true, read: true } : n)); }
    catch (error) { console.error('Failed to mark as read:', error); }
  };

  const markAllAsRead = async () => {
    try { await fetch(`${API_URL}/api/notifications/read-all`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` } }); setNotifications(prev => prev.map(n => ({ ...n, isRead: true, read: true }))); }
    catch (error) { console.error('Failed to mark all as read:', error); }
  };

  const handleNavigate = (notification) => {
    if (!(notification.isRead ?? notification.read)) markAsRead(notification.id);
    ...
    if (!route) {
      ...
      route = config.route;
      const entity = notification.entityId || meta.leadId || meta.dealId || meta.customerId;
                ...
                return (
                  <motion.div key={notification.id} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -50 }} className={`flex items-start gap-4 p-5 cursor-pointer hover:bg-[#FAFAFA] transition-colors ${!notification.isRead ? 'bg-[#F8FAFC]' : ''}`} onClick={() => handleNavigate(notification)} data-testid={`notification-row-${notification.id}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-medium text-[#18181B]">{localizeNotification(notification, t).title || notification.title}</p>
                      {(localizeNotification(notification, t).message || notification.message) && <p className="text-sm text-[#71717A] mt-1">{localizeNotification(notification, t).message || notification.message}</p>}
