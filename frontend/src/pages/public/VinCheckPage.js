...
export default function VinCheckPage() {
  ...
  useSeo({
    title: vin
      ? `VIN ${vin.toUpperCase()} report — BIBI Cars`
      : "Free VIN decoder — instant vehicle history check | BIBI Cars",
    description:
      "Decode any 17-character VIN: make, model, year, engine, equipment, accident & odometer history snapshots — free, instant, no signup.",
    keywords:
      "vin decoder, vin check bulgaria, vehicle history report, decode vin online",
    path: vin ? `/vin-check/${vin}` : "/vin-check",
    alternates: {
      en: "/vin-check?lang=en",
      bg: "/vin-check?lang=bg",
      "x-default": "/vin-check",
    },
  });
  ...
  useEffect(() => {
    ...
    setError(null);
    axios
      .get(`${API}/api/vin/search/${encodeURIComponent(vin)}`)
    ...
    <div data-testid="vin-check-page" className="bg-black min-h-[60vh]">
      <section className="pt-12 pb-20">
        <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px]">
          <Breadcrumbs
            items={[{ label: t.home, to: "/" }, { label: t.vinCheck }]}
          />
          ...
          <form
            onSubmit={submit}
            className="mt-12 flex items-center gap-3 max-w-[640px]"
          >
            <div className="flex-1 flex items-center gap-2 h-12 px-4 border border-[#555452] rounded-lg">
              ...
              <input
                ...
                className="flex-1 bg-transparent outline-none text-[14px] uppercase text-white placeholder:text-[#5E5E5E]"
                data-testid="vin-check-input"
              />
            </div>
            <button className="btn-amber h-12" data-testid="vin-check-submit">
              {t.submit}
            </button>
          </form>
          ...
          {data && (
            <div className="mt-12 bg-[#1D1D1B] rounded-lg p-8">
              ...
              <pre className="text-[12px] text-[#FEAE00] whitespace-pre-wrap break-words">
