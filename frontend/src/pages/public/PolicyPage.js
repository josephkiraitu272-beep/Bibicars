...
const T = {
  en: {
    ...
    back: "← Back to home",
    unavailable: "<p>Content unavailable.</p>",
  },
  bg: {
    home: "Начало /",
    loading: "Зареждане…",
    empty: "Още няма съдържание.",
    back: "← Обратно към началото",
    unavailable: "<p>Съдържанието не е налично.</p>",
  },
};
    ...
    <div className="bibi-policy-page bg-black text-white">
      <section className="bibi-policy-hero">
        <div className="bibi-container">
          ...
          <h1 className="bibi-policy-title">
            {policy?.title || (loading ? t.loading : "")}
          </h1>
        </div>
      </section>
        ...
        <div className="bibi-container">
          <article
            ...
            dangerouslySetInnerHTML={{
              __html:
                policy?.content ||
                (loading ? `<p>${t.loading}</p>` : `<p>${t.empty}</p>`),
            }}
          />
