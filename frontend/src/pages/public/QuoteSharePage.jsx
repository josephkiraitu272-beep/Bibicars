  ...
  const fetchCalc = useCallback(async () => {
    try {
      const r = await axios.get(
        `${API}/api/public/calculations/share/${shareToken}`,
      );
  ...
  const approve = async () => {
    ...
    try {
      const r = await axios.post(
        `${API}/api/public/calculations/share/${shareToken}/approve`,
        {},
      );
      ...
      >
        <div style={{ maxWidth: 540, textAlign: "center" }}>
          ...
          <p style={{ opacity: 0.6, marginBottom: 24 }}>
            {error ||
              "This link is no longer valid or the calculation has been archived."}
          </p>
          <Link
            to="/"
            style={{ color: "#F59E0B", textDecoration: "underline" }}
          >
            ← Back to BIBI Cars home
          </Link>
        </div>
      </div>
    );
    ...
    >
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          >
            Your Import Estimate · v{calc.version || 1}
          </div>
          <h1
            style={{
              fontFamily: "Mazzard H, sans-serif",
          ...
          >
            {(inputs.origin || "usa").toUpperCase()} ·{" "}
            {(inputs.vehicleType || "sedan").toUpperCase()}
            ...
            {breakdown.map((row, i) => (
              ...
              >
                <span
                  style={{
                    fontSize: 15,
                    opacity: row.value < 0 ? 0.9 : 0.85,
                    color: row.value < 0 ? "#22c55e" : "#fff",
                  }}
                >
                  {row.label || row.key}
                  {row.overridden && (
                ...
                >
                  {row.value < 0 ? "−" : ""}
                  {fmt(Math.abs(row.value), row.currency)}
                </span>
              </div>
            ))}
          </div>
        ...
        {comments.length > 0 && (
          ...
          >
            ...
            {comments.map((c) => (
              <div
                key={c.id}
                style={{ padding: "12px 0", borderBottom: "1px solid #27272A" }}
              >
                <div
                  style={{
                    display: "flex",
        ...
        {canApprove && (
          <div style={{ textAlign: "center", padding: "32px 0" }}>
            ...
            <div style={{ opacity: 0.5, fontSize: 13, marginTop: 12 }}>
              Your approval is non-binding — you can still contact your manager
              to adjust the order.
            </div>
          </div>
        )}
        ...
        {isApproved && (
          ...
          >
            ✓ You approved this calculation. Your manager has been notified and
            will be in touch.
          </div>
        )}
        ...
        >
          This calculation is indicative and may vary depending on customs,
          logistics and currency conditions.
          <br />
          FX snapshot: 1 USD ≈ {Number(calc.fx_snapshot || 1).toFixed(3)} EUR ·
          Calculation ID:{" "}
          <span style={{ fontFamily: "monospace" }}>{calc.id}</span>
        </div>
      </div>
    </div>
  );
