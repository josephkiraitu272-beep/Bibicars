...
const Row = ({ label, value }) => (
  <div className="grid grid-cols-[110px_1fr] gap-4 py-2">
    ...
    <span className="text-[14px] font-bold uppercase text-[#FEAE00]">
      {value || "—"}
    </span>
  </div>
);
...
export default function VehicleDetailPage() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${API}/api/vehicles/${encodeURIComponent(id)}`)
      ...
      <section className="pt-12 pb-16">
        <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px]">
          ...
          <div className="flex items-start justify-between gap-6 mt-10">
            <h1
              className="text-[36px] md:text-[64px] font-bold text-white leading-tight"
              data-testid="single-car-title"
            >
              {title}
            </h1>
            <div className="flex items-center gap-4 flex-shrink-0">
              <button
                className="w-10 h-10 rounded-full border border-[#FEAE00] flex items-center justify-center text-[#FEAE00] hover:bg-[#FEAE00] hover:text-black transition-colors"
                aria-label="Compare"
              >
                <Scale size={16} />
              </button>
          ...
          {loading && (
            <div className="py-32 text-center text-[#5E5E5E]">Loading…</div>
          )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-8 mt-12">
              ...
              <div className="bg-[#1D1D1B] rounded-lg p-8 md:p-12 flex flex-col">
                <div className="flex items-start justify-between gap-4 mb-2">
                  ...
                  <div
                    className="px-4 h-9 border border-[#FEAE00] rounded flex items-center text-[14px] font-semibold uppercase text-[#FEAE00]"
                    data-testid="car-status-traded"
                  >
                    Traded
                  </div>
                </div>
                ...
                <h2 className="text-[18px] font-semibold text-white capitalize mt-10">
                  Auction details
                </h2>
                <div className="border-b border-[#555452] my-4" />
                ...
                />
                <button
                  className="btn-amber mt-auto self-center mt-10"
                  data-testid="single-car-exact-cost"
                >
      ...
      <section className="bg-black pb-16">
        <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px]">
          ...
          <div className="text-center mt-12">
            <Link
              to="/catalog"
              className="text-[16px] uppercase underline text-[#FEAE00] hover:brightness-110"
              data-testid="go-back-to-catalog"
            >
              Go back to catalog
            </Link>
          </div>
