...
export default function Sales() {
  ...
  const saveSale = async () => {
    ...
    if (!editor.vin && !editor.lot && !editor.dealId) {
      toast.error("At least one of VIN, Lot or Deal is required");
      return;
    }
    try {
      if (editor.id) {
  ...
  const openHandover = (s) => {
    if (!s.customerId) {
      toast.error(
        tt("saleHandoverNoCustomer", "Sale has no customer attached"),
      );
      return;
    }
    setHandoverForm((f) => ({
  ...
  const generateHandoverAct = async () => {
    ...
    try {
      const r = await axios.post(`${API_URL}/api/sales/${s.id}/handover-act`, {
        ...handoverForm,
        language: "bg",
      });
      const d = r.data?.data || {};
      toast.success(
        `${tt("saleHandoverGenerated", "Handover Act generated")} v${d.version || 1}`,
      );
      ...
      if (d.file_id) {
        try {
          ...
          const res = await fetch(
            `${API_URL}/api/file-manager/files/${d.file_id}/download`,
            { headers },
          );
          ...
          <table className="w-full text-sm min-w-[1100px]">
            <thead className="bg-zinc-50 text-zinc-600 text-[12px] uppercase">
              <tr>
                <th className="text-left px-4 py-3 font-semibold">
                  {t("sales_client") || "Client"}
                </th>
                <th className="text-left px-4 py-3 font-semibold">
                  {t("sales_phone") || "Phone"}
                </th>
                <th className="text-left px-4 py-3 font-semibold">
                  {t("sales_manager") || "Manager"}
                </th>
                <th className="text-left px-4 py-3 font-semibold">
                  {t("sal7_col_sale_date")}
                </th>
            ...
            <tbody className="divide-y divide-zinc-100">
              {loading && filteredItems.length === 0 && (
                <tr>
                  <td colSpan={16} className="text-center py-10 text-zinc-400">
                    Loading…
                  </td>
                </tr>
              )}
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      {s.customerId ? (
                        <a
                          href={`/admin/customers/${s.customerId}/360`}
                          className="text-zinc-900 font-medium hover:text-[#4F46E5]"
                          data-testid={`sale-customer-${s.id}`}
                        >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap font-mono text-[12px]">
                      {s.customerPhone ? (
                        <a
                          href={`tel:${String(s.customerPhone).replace(/\s+/g, "")}`}
                    ...
                    <td className="px-4 py-3 text-zinc-700 whitespace-nowrap">
                      {s.managerName ||
                        (s.managerId ? s.managerId.slice(-6) : "—")}
                    </td>
                    ...
                    <td className="px-4 py-3 font-semibold text-zinc-900">
                      {Number(s.saleAmount || 0).toLocaleString()}{" "}
                      {s.saleCurrency}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold ${badge.bg} ${badge.text}`}
                      >
                        {badge.label}
                      </span>
                    </td>
                      ...
                      (s.utm.utm_source ||
                        ...
                        >
                          ...
                          <span className="truncate">
                            {[
                              s.utm.utm_source,
                              s.utm.utm_medium ? `/ ${s.utm.utm_medium}` : "",
                              s.utm.utm_campaign
                                ? `· ${s.utm.utm_campaign}`
                                : "",
                            ]
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="inline-flex items-center gap-1">
                        ...
                        <button
                          ...
                          title={tt(
                            "saleHandoverAct",
                            "Свали Приёмо-предавателен протокол",
                          )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[92vh] overflow-y-auto">
              ...
              {editor.source === "deal" && (
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500 mt-1">
                    When set, the server enriches VIN/Lot/Brand/Model/Year from
                    the deal.
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    value={editor.lot || ""}
                    onChange={(e) =>
                      setEditor({ ...editor, lot: e.target.value })
                    }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditor({ ...editor, country: e.target.value })
                    }
                    className="w-full h-10 px-3 rounded-xl border border-zinc-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-black/10"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditor({ ...editor, auction: e.target.value })
                    }
                    className="w-full h-10 px-3 rounded-xl border border-zinc-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-black/10"
                ...
                <div>
                  ...
                  <input
                    ...
                    value={editor.year || ""}
                    onChange={(e) =>
                      setEditor({ ...editor, year: e.target.value })
                    }
                    className="w-full h-10 px-3 rounded-xl border border-zinc-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-black/10"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditor({ ...editor, saleCurrency: e.target.value })
                    }
                    className="w-full h-10 px-3 rounded-xl border border-zinc-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-black/10"
            ...
            <div className="sticky bottom-0 bg-white border-t border-zinc-200 px-6 py-4 flex items-center justify-end gap-2">
              ...
              >
                ...
                {editor.id ? "Save changes" : "Create sale"}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-xl w-full my-8">
            <div className="px-5 py-4 border-b border-[#E4E4E7] flex items-center justify-between sticky top-0 bg-white rounded-t-2xl z-10">
              <div>
                <h3 className="text-base font-bold text-[#18181B]">
                  {tt("handoverActTitle", "Приёмо-предавателен протокол")}
                </h3>
                <p className="text-[11px] text-[#71717A] mt-0.5">
                  {tt(
                    "handoverActSubtitle",
                    "Приложение №2 към Договор за поръчка",
                  )}{" "}
                  ·{handoverModal.sale?.brand} {handoverModal.sale?.model}{" "}
                  {handoverModal.sale?.year
                    ? `· ${handoverModal.sale.year}`
                    : ""}
                </p>
              </div>
            ...
            <div className="px-5 py-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] uppercase tracking-wider font-bold text-[#71717A] mb-1">
                    {tt("handoverPlace", "Място на предаване")}
                  </label>
                  <input
                    type="text"
                    value={handoverForm.handover_place}
                    onChange={(e) =>
                      setHandoverForm({
                        ...handoverForm,
                        handover_place: e.target.value,
                      })
                    }
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-wider font-bold text-[#71717A] mb-1">
                  {tt("handoverDocuments", "Предадени документи")}
                </label>
                <textarea
                  value={handoverForm.documents}
                  onChange={(e) =>
                    setHandoverForm({
                      ...handoverForm,
                  ...
                  placeholder={tt(
                    "handoverDocumentsPh",
                    "напр. Договор за покупка, талон, ключове, COC, митническа декларация",
                  )}
                  rows={2}
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-wider font-bold text-[#71717A] mb-1">
                  {tt("handoverCondition", "Състояние при предаване")}
                </label>
                <textarea
                  ...
                  placeholder={tt(
                    "handoverConditionPh",
                    "Без видими повреди / описание на установени забележки",
                  )}
                  rows={2}
                  className="w-full px-3 py-2 rounded-lg border border-[#E4E4E7] bg-white text-[13px]"
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-wider font-bold text-[#71717A] mb-1">
                  {tt("handoverKeys", "Ключове / аксесоари")}
                </label>
                <input
                  type="text"
                  value={handoverForm.keys_and_accessories}
                  onChange={(e) =>
                    setHandoverForm({
                      ...handoverForm,
                      keys_and_accessories: e.target.value,
                    })
                  }
                  placeholder={tt(
                    "handoverKeysPh",
                    "2 ключа, ръководство, гумени постелки",
                  )}
                  className="w-full h-9 px-3 rounded-lg border border-[#E4E4E7] bg-white text-[13px]"
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-wider font-bold text-[#71717A] mb-1">
                  {tt("handoverRemarks", "Забележки на ВЪЗЛОЖИТЕЛЯ")}
                </label>
                <textarea
                  value={handoverForm.comments}
                  onChange={(e) =>
                    setHandoverForm({
                      ...handoverForm,
                  ...
                  placeholder={tt(
                    "handoverRemarksPh",
                    "Допълнителни забележки (по избор)",
                  )}
                  rows={2}
                  className="w-full px-3 py-2 rounded-lg border border-[#E4E4E7] bg-white text-[13px]"
